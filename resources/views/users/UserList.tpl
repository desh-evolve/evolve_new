{include file="header.tpl" enable_ajax=TRUE body_onload=" "}
				...
				{foreach from=$users item=user name=user}
					...
					<tr class="{$row_class}">
						...
						<td id="col_user_funcs">
                         ...
                         <!-- ARSP NOTE -> I ADDED THIS CODE FOR TUNDER & NEON FOR JOB HISTORY --> 
						...
						{if $permission->Check('wage','view') OR ( $permission->Check('wage','view_child') AND $user_data.is_child === TRUE ) OR ( $permission->Check('wage','view_own') AND $user_data.is_owner === TRUE )}
							[ <a href="{urlbuilder script="UserJobHistoryList.php" values="user_id=$user_id,saved_search_id=$saved_search_id" merge="FALSE"}" onClick="return isModifiedForm();">{t}Job History{/t}</a> ]
						{/if} 
{*						FL ADDED FOR NATIONAL PVC 20160627*}
						{if $permission->Check('user', 'medical_aid_scheme') OR ( $permission->Check('user', 'medical_aid_scheme') AND $user_data.is_child === TRUE ) OR ( $permission->Check('user', 'medical_aid_scheme') AND $user_data.is_owner === TRUE )}
							[ <a href="{urlbuilder script="../users/MedicalClaimList.php" values="user_id=$user_id,saved_search_id=$saved_search_id" merge="FALSE"}" onClick="return isModifiedForm();">{t}Medical Claims{/t}</a> ]
						{/if} 	
						
{*						FL ADDED FOR NATIONAL PVC 20160627*}
						{if $permission->Check('user','personal_date_update_form') OR ( $permission->Check('user','personal_date_update_form') AND $user_data.is_child === TRUE ) OR ( $permission->Check('user','personal_date_update_form') AND $user_data.is_owner === TRUE )}
							[ <a href="{urlbuilder script="../users/UserDateUpdateForm.php" values="user_id=$user_id,saved_search_id=$saved_search_id" merge="FALSE"}" onClick="return isModifiedForm();">{t}Date Update{/t}</a> ]
						{/if}                                  
						{if $permission->Check('wage','view') OR ( $permission->Check('wage','view_child') AND $user_data.is_child === TRUE ) OR ( $permission->Check('wage','view_own') AND $user_data.is_owner === TRUE )}
							[ <a href="{urlbuilder script="../kpi/KpiUserList.php" values="user_id=$user_id,saved_search_id=$saved_search_id" merge="FALSE"}" onClick="return isModifiedForm();">{t}KPI{/t}</a> ]
						{/if} 						
