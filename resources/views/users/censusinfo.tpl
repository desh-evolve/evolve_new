...
<div id="rowContentInner">
		...
		<form method="get" name="accrual_balance" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Dependants{/t}{/capture}
					...
					<td>
						...
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
                                        ...
                                        <td>
						{capture assign=label}{t}Relationship{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
                                        <td>
						{capture assign=label}{t}DOB{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
                                        <td>
						{capture assign=label}{t}NIC{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
                                        <td>
						{capture assign=label}{t}Gender{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
					<td>
						{t}Functions{/t}
				...
				{foreach from=$censuses item=census name=census}
					{cycle assign=row_class values="tblDataWhite,tblDataGrey"}
					{if $census.deleted == TRUE}
						{assign var="row_class" value="tblDataDeleted"}
					{/if}
					<tr class="{$row_class}">
						<td>
							{$smarty.foreach.census.iteration}
						</td>										
						<td>
							{$census.dependant}
						</td>
						<td>
                                                ...
                                                <td>
                                                        {getdate type="DATE" epoch=$census.dob}
						</td>
                                                <td>
                                                        {$census.nic}
						</td>
						...
						<td>
							{assign var="id" value=$census.id}
							{assign var="user_id" value=$census.user_id}
							...
							{if ( $permission->Check('accrual','view') OR ($permission->Check('accrual','view_child') AND $is_child === TRUE) OR ($permission->Check('user','view_own') AND $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="editCensus.php" values="user_id=$user_id,id=$id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
					</tr>
				{/foreach}
