...
<div id="rowContentInner">
		...
		<form method="get" name="userwage" action="{$smarty.server.SCRIPT_NAME}">
		  ...
		  <tr class="tblHeader">
				  <td>
						{capture assign=label}{t}Year / Date{/t}{/capture}
				  ...
				  <td>
						{capture assign=label}{t}EPF #{/t}{/capture}
					...
					<tr class="{$row_class}">
						...
						<td>
							...
							{if $permission->Check('wage','edit') OR ( $permission->Check('wage','edit_child') AND $job.is_child === TRUE ) OR ( $permission->Check('wage','edit_own') AND $job.is_owner === TRUE )}
								[ <a href="{urlbuilder script="EditUserDateUpdateForm.php" values="id=$job_id,saved_search_id=$saved_search_id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
						<td>
