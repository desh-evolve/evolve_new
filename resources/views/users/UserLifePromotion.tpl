...
<div id="rowContentInner">
		...
		<form method="get" name="accrual_balance" action="{$smarty.server.SCRIPT_NAME}">
				...
				{foreach from=$lifepromotions item=lifepromotion name=lifepromotion}
					{cycle assign=row_class values="tblDataWhite,tblDataGrey"}
					{if $lifepromotion.deleted == TRUE}
						{assign var="row_class" value="tblDataDeleted"}
					{/if}
					<tr class="{$row_class}">
						<td>
							{$smarty.foreach.lifepromotion.iteration}
						</td>										
						<td>
							{$lifepromotion.current_designation}
						</td>
						<td>
                                                      
                                                         {$lifepromotion.new_designation}
						</td>
                                                <td>
                                                         {$lifepromotion.current_salary}
						</td>
                                                 <td>
                                                        {$lifepromotion.new_salary}
						</td>
                                                 <td>
                                                        {getdate type="DATE" epoch=$lifepromotion.effective_date}
						</td>
                                                
                                                
                                              
						<td>
							{assign var="id" value=$lifepromotion.id}
							{assign var="user_id" value=$lifepromotion.user_id}
							...
							{if ( $permission->Check('accrual','view') OR ($permission->Check('accrual','view_child') AND $is_child === TRUE) OR ($permission->Check('user','view_own') AND $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="EditUserLifePromotion.php" values="user_id=$user_id,id=$id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
					</tr>
				{/foreach}
