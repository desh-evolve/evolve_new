...
<div id="rowContentInner">
		...
		<form method="get" name="accrual_balance" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						...
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
                                        ...
                                        <td>
						{capture assign=label}{t}Designation{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
                                         <td>
				...
				{foreach from=$workexperionce item=workexperionce name=workexperionce}
					{cycle assign=row_class values="tblDataWhite,tblDataGrey"}
					{if $workexperionce.deleted == TRUE}
					...
					<tr class="{$row_class}">
						...
						<td>
                                                         {getdate type="DATE" epoch=$workexperionce.from_date}
						</td>
                                                <td>
                                                        {getdate type="DATE" epoch=$workexperionce.to_date}
						</td>
                                                 <td>
                                                        {$workexperionce.department}
						</td>
                                                 <td>
                                                        {$workexperionce.designation}
						</td>
                                                <td>
						...
						<td>
							...
							{if ( $permission->Check('accrual','view') OR ($permission->Check('accrual','view_child') AND $is_child === TRUE) OR ($permission->Check('user','view_own') AND $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="EditUserWorkExperionce.php" values="user_id=$user_id,id=$id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
						</td>
					</tr>
				{/foreach}
