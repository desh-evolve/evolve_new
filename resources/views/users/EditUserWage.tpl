...
var hwCallback = {
		getHourlyRateAqua: function(result) {
...
function getHourlyRateAqua() {
	...
	if ( document.getElementById('type_id').value != 10 ) {
		remoteHW.getHourlyRateAqua( document.getElementById('wage_val').value, document.getElementById('weekly_time_val').value, document.getElementById('type_id').value, document.getElementById('user_id_val').value);
	}
}
		...
		<form method="post" name="wage" action="{$smarty.server.SCRIPT_NAME}">
		    <div id="contentBoxTwoEdit">
				...
				<tr>
					...
					<td class="cellRightEditTable">
						<select id="type_id" name="wage_data[type]" onChange="showWeeklyTime('weekly_time'); getHourlyRateAqua(); ">
				...
				<tr>
					...
					<td class="cellRightEditTable">
						{$wage_data.currency_symbol}<input id="wage_val" size="15" type="text" name="wage_data[wage]" value="{$wage_data.wage}" onChange="getHourlyRateAqua()"> {$wage_data.iso_code}
					</td>
				</tr>
					...
					<td class="cellRightEditTable">
						{$wage_data.currency_symbol}<input id="wage_val" size="15" type="text" name="wage_data[budgetary_allowance]" value="{$wage_data.budgetary_allowance}" onChange="getHourlyRateAqua()" readonly> {$wage_data.iso_code}
					</td>
				</tr>
