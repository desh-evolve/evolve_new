...
<div id="rowContentInner">
		...
		<form method="get" name="accrual_balance" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						...
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
                                        ...
                                        <td>
						{capture assign=label}{t}Y number{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
				...
				{foreach from=$bonuses item=bonus name=bonus}
					{cycle assign=row_class values="tblDataWhite,tblDataGrey"}
					{if $bonus.deleted == TRUE}
						{assign var="row_class" value="tblDataDeleted"}
					{/if}
					<tr class="{$row_class}">
						<td>
							{$smarty.foreach.bonuses.iteration}
						</td>										
						<td>
							{$bonus.id}
						</td>
						<td>
                                                         {getdate type="DATE" epoch=$bonus.start_date}
						</td>
                                                <td>
                                                        {getdate type="DATE" epoch=$bonus.end_date}
						</td>
                                                <td>
						...
						<td>
							{assign var="id" value=$bonus.id}
							

							{if ( $permission->Check('company','enabled') OR ($permission->Check('company','view_child') AND $is_child === TRUE) OR ( $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="EditBonusCalc.php" values="id=$id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
                                                        
                                                        {if ( $permission->Check('company','enabled') OR ($permission->Check('company','view_child') AND $is_child === TRUE) OR ( $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="EditBonusCalc.php" values="id=$id,view=true" merge="FALSE"}">{t}View{/t}</a> ]
							{/if}
                                                        {if ( $permission->Check('company','enabled') OR ($permission->Check('company','view_child') AND $is_child === TRUE) OR ( $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="BonusList.php" values="dec_bo_id=$id,action=view" merge="FALSE"}">{t}List{/t}</a> ]
							{/if}
						</td>
					</tr>
				{/foreach}
