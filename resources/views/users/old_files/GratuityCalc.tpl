...
<div id="rowContentInner">
		...
		<form method="get" name="accrual_balance" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Company{/t}{/capture}
					...
					<tr class="{$row_class}">
						...
						<td>
							...
							{if ( $permission->Check('company','enabled') OR ($permission->Check('company','view_child') AND $is_child === TRUE) OR ( $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="EditAttendanceBonusCalc.php" values="id=$id" merge="FALSE"}">{t}Edit{/t}</a> ]
							{/if}
                                                        
                                                        {if ( $permission->Check('company','enabled') OR ($permission->Check('company','view_child') AND $is_child === TRUE) OR ( $is_owner === TRUE ))}
								[ <a href="{urlbuilder script="EditAttendanceBonusCalc.php" values="id=$id,view=true" merge="FALSE"}">{t}View{/t}</a> ]
							{/if}
                                                        {if ( $permission->Check('company','enabled') OR ($permission->Check('company','view_child') AND $is_child === TRUE) OR ( $is_owner === TRUE ))}
