...
<div id="rowContentInner">
		...
		<form method="get" name="accrual_balance" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr class="tblHeader">
					...
					<td>
						{capture assign=label}{t}Employee Number{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
                                        <td>
						{capture assign=label}{t}Employee Name{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
                                        <td>
						{capture assign=label}{t}Bonus Amount{/t}{/capture}
						{include file="column_sort.tpl" label=$label sort_column="a.balance" current_column="$sort_column" current_order="$sort_order"}
					</td>
				...
				{foreach from=$data item=bonus name=bonus}
