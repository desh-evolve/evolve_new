<x-app-layout :title="'Input Example'">
    ...
    <style>
        .custom-padding-row th,
        .custom-padding-row td {
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header align-items-center d-flex justify-content-between">
                    ...
                    <a href="/schedule/recurring_schedule_template_control_list" class="btn btn-primary">Recurring Schedule Template List <i class="ri-arrow-right-line"></i></a>
                </div>
                    ...
                    <form method="POST" action="{{ isset($data['id']) ? route('schedule.edit_recurring_schedule_template.submit', $data['id']) : route('schedule.edit_recurring_schedule_template.submit') }}">
                        ...
                        <div id="contentBoxTwoEdit">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr class="custom-padding-row">
                                    <th>Name:</th>
                                    <td class="cellRightEditTable">
                                        <input type="text" class="form-control form-control-sm w-50" name="data[name]" value="{{ $data['name'] ?? '' }}">
                                    </td>
                                </tr>
                                ...
                                <tr class="custom-padding-row">
                                    <th>Description:</th>
                                    <td class="cellRightEditTable">
                                        <input type="text" class="form-control form-control-sm w-50" name="data[description]" value="{{ $data['description'] ?? '' }}">
                                    </td>
                                </tr>
                                    ...
                                    <td colspan="2">
                                        <table class="table table-bordered">
                                            ...
                                            @foreach ($week_rows as $week_row)
                                                <tr class="">
                                                    <td>
                                                        <input type="text" class="form-control form-control-sm" size="4" name="week_rows[{{$week_row['id']}}][week]" value="{{ $week_row['week'] }}">
                                                    ...
                                                    <td>
                                                        <select name="week_rows[{{$week_row['id']}}][schedule_policy_id]" class="form-select form-select-sm" id="schedule_policy_id-{{$week_row['id']}}" onChange="getRecurringScheduleTotalTime({{$week_row['id']}});">
                                                            @foreach ($data['schedule_options'] as $id => $label)
                                                                <option value="{{ $id }}" {{ $id == $week_row['schedule_policy_id'] ? 'selected' : '' }}>
                                                                    {{ $label }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td>
                                                        <select class="form-select form-select-sm" id="branch_id" name="week_rows[{{$week_row['id']}}][branch_id]">
                                                            @foreach ($data['branch_options'] as $id => $label)
                                                                <option value="{{ $id }}" {{ $id == $week_row['branch_id'] ? 'selected' : '' }}>
                                                                    {{ $label }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td>
                                                        <select class="form-select form-select-sm" id="department_id" name="week_rows[{{$week_row['id']}}][department_id]">
                                                            @foreach ($data['department_options'] as $id => $label)
                                                                <option value="{{ $id }}" {{ $id == $week_row['department_id'] ? 'selected' : '' }}>
                                                                    {{ $label }}
                                                                </option>
                                                            @endforeach
                                                        </select>
