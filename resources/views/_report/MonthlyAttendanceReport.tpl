...
<div id="rowContentInner">
		<form method="post" name="report" action="{$smarty.server.SCRIPT_NAME}" target="_self">
		    ...
		    <div id="contentBoxTwoEdit">
				...
				<select name="filter_data[column_ids][]" id="filter_column" hidden="" style="width:90%;margin:5px 0 5px 0;" size="4" multiple>
                        <option label="Date" value="-1000-date_stamp">Date</option>
						<option label="Worked Time" value="-1090-worked_time">Worked Time</option>
						<option label="Paid Time" value="-1130-paid_time">Paid Time</option>
						<option label="Regular Time" value="-1140-regular_time">Regular Time</option>
                </select>


				<!-- {htmlreportsort filter_data=$filter_data}  

				<select id="columns" name="filter_data[primary_sort]" hidden="">
			...
			<div id="contentBoxFour">
				<input type="submit" {$hidden_elements.displayReport} name="BUTTON" value="{t}Display Report{/t}" onClick="selectAllReportCriteria(); this.form.target = '_blank'; document.getElementById('action').name = 'action:Display Report';">
				<input type="submit" {$hidden_elements.displayTimeSheet} name="BUTTON" value="{t}Display TimeSheet{/t}" onClick="selectAllReportCriteria(); document.getElementById('action').name = 'action:Display TimeSheet';">
				<input type="submit" {$hidden_elements.displayDetailedTimeSheet} name="BUTTON" value="{t}Display Detailed TimeSheet{/t}" onClick="selectAllReportCriteria(); document.getElementById('action').name = 'action:Display Detailed TimeSheet';">
				<input type="submit" {$hidden_elements.export} name="BUTTON" value="{t}Export{/t}" onClick="selectAllReportCriteria(); this.form.target = '_self'; document.getElementById('action').name = 'action:Export';">
			</div>
