...
var JSCalendarPayPeriodTransactionDates = <?php echo Misc::getJSArray( $transactionDate )."\n"; ?>
...
function Birthday_popups() {

    //try {
	//	tP=window.open('<?php //echo Environment::getBaseURL();?>company/BirthdayReminders.php',"Time_Punch","toolbar=0,status=1,menubar=0,scrollbars=1,fullscreen=no,width=600,height=470,resizable=1");
	//	if (window.focus) {
	//		tP.focus()
	//	}
	//} catch (e) {
...
function viewLeave(id) {
	try {
		tP=window.open('<?php echo Environment::getBaseURL();?>leaves/VIewUserLeave.php?id='+id,"Time_Punch","toolbar=0,status=1,menubar=0,scrollbars=1,fullscreen=no,width=600,height=470,resizable=1");
		if (window.focus) {
			tP.focus()
		}
	} catch (e) {
...
function viewNumberLeave(id) {
	try {
		tP=window.open('<?php echo Environment::getBaseURL();?>leaves/VIewNumberOfLeave.php?id='+id,"Time_Punch","toolbar=0,status=1,menubar=0,scrollbars=1,fullscreen=no,width=800,height=470,resizable=1");
		if (window.focus) {
			tP.focus()
		}
	} catch (e) {
...
function confirmSubmit(text) {
	...
	if ( text == null ) {
		text = "<?php echo 'You are about to delete data, once data is deleted it can not be recovered.\nAre you sure you wish to continue?'; ?>";
	}
...
function singleSubmitHandler( button ) {
	...
	if ( submitcount[button_name] == 0 ) {
		...
		alert('<?php echo 'You have already submitted this form! Please wait...'; ?>');
		return false;
	}
}
...
function SubmitWithMessage(form) {
		var alertMsg= "<?php echo 'Frame data DOES NOT exists:'; ?> ";
...
function newMailPopUp(base_url) {
		var resultMsg = "<?php echo 'You have new mail waiting. Would you like to read it now?'; ?>";
...
function isModifiedForm() {
	var msg = "<?php echo 'You have modified data without saving, if you press OK you will lose any changes you have made!'; ?>";
...
function ImportCsvFile(object_type,object_id) {
//alert('obj_type: ' + object_type);
	try {
		uS=window.open('<?php echo Environment::getBaseURL();?>upload/ImportCsvFile.php?object_type='+ object_type +'&object_id='+ object_id,"File_Import","toolbar=0,status=1,menubar=0,scrollbars=2,fullscreen=no,width=400,height=250,resizable=1");
		if (window.focus) {
			fU.focus()
		}
	} catch (e) {
