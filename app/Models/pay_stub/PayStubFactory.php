...
//                    $adjust_y -= 2; //fl added for rosen 
                    ...
                    $pdf->Cell(175, 3, TTi18n::gettext('PAY SLIP - ' . strtoupper(date('M, Y', $pp_end_date))), $border, 0, 'L', 0);
                    $pdf->setLineWidth(0.75);
...
//                    
                    if (isset($pay_stub_entries[10])) {//description availability check
                        ...
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext("EPF No."), $border, 0, 'L');
                        $pdf->Cell($column_widths['amount'], 20, ": ".TTi18n::gettext($user_obj->getEmployeeNumber()), $border, 0, 'L');

                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y, $adjust_y + 5));//(0,20) - (30,2)
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Name'), $border, 0, 'L');
                        $pdf->Cell($column_widths['amount'], 20, ": ".TTi18n::gettext($user_obj->getFullName()), $border, 0, 'L');

                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y, $adjust_y + 10));//(0,20) - (30,7)
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Designation'), $border, 0, 'L');
                        $pdf->Cell($column_widths['amount'], 20, ": ".TTi18n::gettext(Option::getByKey($user_obj->getTitle(), $title_options, NULL )), $border, 0, 'L');

                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y, $adjust_y + 15));//(0,20) - (30,12)
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Department'), $border, 0, 'L');
                        $pdf->Cell($column_widths['amount'], 20, ": ".TTi18n::gettext(Option::getByKey($user_obj->getDefaultDepartment(), $department_options, NULL )), $border, 0, 'L');

                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y, $adjust_y + 20));//(0,20) - (30,17)
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Location'), $border, 0, 'L');         
                        $pdf->Cell($column_widths['amount'], 20, ": ".TTi18n::gettext(Option::getByKey($user_obj->getDefaultBranch(), $branch_options, NULL )), $border, 0, 'L');

                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y, $adjust_y + 25));//(0,20) - (30,22)
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Days Worked'), $border, 0, 'L');
                        ...
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('NoPay Days'), $border, 0, 'L');
                        ...
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Normal OT Hrs'), $border, 0, 'L');
                        ...
                        $pdf->Cell($column_widths['name'], 20, TTi18n::gettext('Holiday OT Hrs'), $border, 0, 'L');
                        ...
                        $pdf->Cell($column_widths['name']+2, 23, TTi18n::gettext(' '), $border, 0, 'L');
                        $pdf->Cell($column_widths['amount'], 20, TTi18n::gettext('LKR'), $border, 0, 'L');
...
////                        $pdf->SetFont('', 'B', 10);
////                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y, $adjust_y));
////                        $pdf->Cell($column_widths['amount'], 5, TTi18n::gettext('Accruals'), $border, 0, 'L');
////                        $pdf->Cell($column_widths['amount'], 5,  ": ".TTi18n::gettext('Amount'), $border, 0, 'L');
////
////                        $block_adjust_y += 5;
////
//                        $pdf->SetFont('', '', 10);
...
/*                    //
                    ...
                    if ($ablf->getRecordCount() > 0) {
                        ...
                        $pdf->Cell(25, 5, TTi18n::gettext('Balance (hrs)'), $border, 0, 'L');
...
*/
                ...
                //Salary Receipt
                    ...
                    $pdf->Cell(100, 1, TTi18n::gettext('SALARY RECEIPT'), $border, 0, 'L', 0);
                    ...
                    $pdf->Cell(100, 1, TTi18n::gettext($user_obj->getEmployeeNumber() . '  ' . $title . $user_obj->getNameWithInitials()), $border, 0, 'L', 0);

                    $pdf->SetFont('', '', 10);

                    $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y + 145, $adjust_y));

                    $pdf->Cell(95, 1, TTi18n::gettext('Received the undermentioned amount being balance pay'), $border, 0, 'L', 0);
                    
                    $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y + 150, $adjust_y));
                    $pdf->Cell(40, 1, TTi18n::gettext('due to me for the month of '), $border, 0, 'L', 0);
                    $pdf->Cell(40, 1, TTi18n::gettext(strtoupper(date('F, Y', $pp_end_date))), $border, 0, 'L', 0);

                    $pdf->SetFont('', 'B', 10);

                    $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y + 155, $adjust_y));
                    $pdf->Cell($column_widths['name']+5, 1, TTi18n::gettext(' '), $border, 0, 'L', 0);
                    $pdf->Cell($column_widths['amount'], 1, TTi18n::gettext('Rs.'), $border, 0, 'L', 0);

                    $pdf->SetFont('', '', 10);

                    $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y + 160, $adjust_y));

                    $pdf->Cell($column_widths['name'], 1, TTi18n::gettext('Salary & Other Income'), $border, 0, 'L', 0);
...
//                    print_r(TTi18n::formatNumber($performance_Incentive)); die;
                    if(isset($performance_Incentive)){
                        $pdf->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY($block_adjust_y + 165, $adjust_y));
                        $pdf->Cell($column_widths['name'], 1, TTi18n::gettext('Performance Incentive'), $border, 0, 'L', 0);
                    ...
                    $pdf->Cell($column_widths['name'], 1, TTi18n::gettext('Total Income'), $border, 0, 'L', 0);
                    ...
                    $pdf->Cell(50, 1, TTi18n::gettext('Pay Stub Generated by') . ' ' . APPLICATION_NAME, $border, 0, 'C', 0);
...
//			$pdf = new TTPDF('P','mm','A4');//----@widanage change code here----17.04.2013
			...
			foreach ($pslf as $pay_stub_obj) {
				...
				$pdf->Line( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(15, $adjust_y), Misc::AdjustXY(193, $adjust_y), Misc::AdjustXY(15, $adjust_y) );
                                ...
                                $pdf->Cell(20,40,TTi18n::gettext('Employee Number').' ', $border, 0, 'L');
                                
                                
                                $pdf->setXY( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(5, $adjust_y) );
                                $pdf->Cell(20,40,TTi18n::gettext('Employee Name').' ', $border, 0, 'L');
                                
                                
                                $pdf->setXY( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(10, $adjust_y) );
                                $pdf->Cell(20,40,TTi18n::gettext('Designation').' ', $border, 0, 'L');
                                
                                
                                $pdf->setXY( Misc::AdjustXY(30, $adjust_x), Misc::AdjustXY(0, $adjust_y) );
                                $pdf->Cell(20,40,TTi18n::gettext(':').' ', $border, 0, 'L');
                                ...
                                $pdf->Cell(20,40,TTi18n::gettext('Month/Year').' ', $border, 0, 'L');
                                
                                $pdf->setXY( Misc::AdjustXY(110, $adjust_x), Misc::AdjustXY(5, $adjust_y) );
                                $pdf->Cell(20,40,TTi18n::gettext('Date of payment').' ', $border, 0, 'L');
    ...
    function FormE_payments($data, $include_user_ids , $current_user, $current_company, $payperiod_string,$branchid, $export_type, $submission_number)
        {
            ...
            if ( is_array($data))
            {
                ...
                $pdf = TTnew( 'PayStubListFactoryFormE_payments' );			
                ...
                $cfsf = TTnew( 'CformSubmissionFactory' );    
                   
                $clf1 = TTnew( 'CompanyListFactory' ); 
	   ...
	   function Form_ETF($data, $include_user_ids , $current_user, $current_company, $payperiod_string,$branchid, $export_type, $submission_number)
        {        
            ...
            if ( is_array($data) AND count($include_user_ids) > 0  )
            {
                ...
                $pdf = TTnew( 'PayStubListFactoryForm_ETF' );			
    ...
    function FormE_epf($data, $include_user_ids , $current_user, $current_company, $payperiod_string,$branchid, $export_type, $submission_number)
	 ...
	 //echo "<h1>Dear Sir, If you see this message please inform to me(Prashanthan)</h1><p/>";
            ...
            if ( is_array($data) AND count($include_user_ids) > 0  )
            {
                ...
                $pdf = TTnew( 'PayStubListFactoryFormE_epf' );			
