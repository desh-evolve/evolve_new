...
/*
 * $Revision: 5520 $
 * $Id: UserDateTotalFactory.class.php 5520 2011-11-15 22:47:55Z ipso $
 * $Date: 2011-11-15 14:47:55 -0800 (Tue, 15 Nov 2011) $
 */
...
class UserDateTotalFactory extends Factory {
	...
	function calcOverTimePolicyTotalTime( $udt_meal_policy_adjustment_arr, $udt_break_policy_adjustment_arr ) {
		...
		//echo '<br>schedule_total_time'.gmdate("H:i:s", $schedule_total_time);
		//echo '<br><br>';
		...
		//echo 'otp_calculation_order'; print_r($otp_calculation_order);
		//echo '<br><br>';
		...
		if ( $otplf->getRecordCount() > 0 ) {
			...
			$tmp_trigger_time_arr = array(); $npvc_daily = false;
			foreach( $otplf as $otp_obj ) {
				...
				//echo '<br><br>getType....'.$otp_obj->getType();
			...
			//echo '<br><br>trigger_time_arr2<br>';
			//print_r($trigger_time_arr);
		...
		if ( isset($trigger_time_arr) ) {
			...
			if ( $udtlf->getRecordCount() > 0 ) {
				...
				foreach( $udtlf as $udt_obj ) {
					...
					//echo '<br>trigger_time_arr3....'; print_r( $trigger_time_arr);
					...
					foreach( $trigger_time_arr as $trigger_time_data ) {
						...
						//echo '1....'.$total_daily_hours.' > '.$trigger_time_arr[$i]['trigger_time'];

						//echo '<br>2...'.gmdate("H:i:s", $total_daily_hours).' > '.gmdate("H:i:s", $trigger_time_arr[$i]['trigger_time']);
						...
						if ( $total_daily_hours > $trigger_time_arr[$i]['trigger_time'] ) {
							...
							if($npvc_daily)//
							{
								if($schedule_start_time>0 && $trigger_time_arr[$i]['over_time_policy_type_id']==40)
								{
								...
								elseif($schedule_start_time>0)
								{
									...
									echo '<br><br>schedule_start_time...'.$schedule_start_time;
									...
									//echo '<br><br>worked time...'.$date_added->format('Y-m-d H:i:s').'....end time....'.$cal_end_time;

									//Add another 30mins
									$date_added->add(new DateInterval('PT30M'));

									//echo '<br><br>worked time+30...'.$date_added->format('Y-m-d H:i:s').'....end time....'.$cal_end_time;
