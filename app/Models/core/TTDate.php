...
class TTDate {
	...
	public static function setTimeZone($time_zone = NULL) {
		...
		$time_zone = 'Asia/Colombo';//--------@widanage add code and comment above row for change the time zone of the system----03.06.2013
	...
	public static function getDSTOffset( $start_epoch, $end_epoch ) {
		Debug::text('Start Epoch: '. TTDate::getDate('DATE+TIME', $start_epoch) .'('.date('I', $start_epoch).')  End Epoch: '. TTDate::getDate('DATE+TIME', $end_epoch) .'('.date('I', $end_epoch).')', __FILE__, __LINE__, __METHOD__, 10);
		if ( date('I', $start_epoch) == 0 AND date('I', $end_epoch) == 1 ) {
			return -3600; //DST==TRUE: Spring - Spring ahead an hour, which means we minus one hour from the offset.
		} elseif ( date('I', $start_epoch) == 1 AND date('I', $end_epoch) == 0 ) {
			return 3600; //DST==FALSE: Fall - Fall back an hour, which means we add one hour to the offset
		}
	...
	public static function getTimePeriodDates( $time_period, $epoch = NULL, $user_obj = NULL, $params = NULL ) {
		...
		switch( $time_period ) {
			...
			case 'last_pay_period':
				...
				Debug::Arr($pay_period_ids, 'aPay Period IDs: ('. (int)$pplf->getRecordCount().')', __FILE__, __LINE__, __METHOD__,10);
				break;
			case 'this_month':
		...
		if ( $post_processing == TRUE ) {
			...
			switch( $column ) {
				...
				case 'pay_period_transaction_date':
					...
					break;
				case 'date_stamp':
					$retval = TTDate::getDate( 'DATE', strtotime($epoch) );
					break;
				case 'time_stamp':
