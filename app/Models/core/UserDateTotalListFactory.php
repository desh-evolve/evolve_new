...
class UserDateTotalListFactory extends UserDateTotalFactory implements IteratorAggregate {
	...
	function getByUserDateIdAndStatusAndTypeAndPunchControlIdAndOverride($user_date_id, $status, $type, $punch_control_id, $override = FALSE, $order = NULL) {
		...
		$query = '
					...
					where	a.user_date_id = :user_date_id
						...
						AND a.type_id = :type
						AND a.punch_control_id = :punch_control_id
						AND a.override = :override
						AND a.deleted = 0
					';
		$query .= $this->getSortSQL( $order, $strict );
	...
	function getByUserDateIdAndStatusAndTypeAndOverTimePolicyIdAndOverride($user_date_id, $status, $type, $over_time_policy_id, $override = FALSE, $order = NULL) {
		...
		$query = '
					...
					where	a.user_date_id = :user_date_id
						AND a.status_id = :status
						AND a.type_id = :type
						AND a.over_time_policy_id = :over_time_policy_id
						AND a.override = :override
						AND a.deleted = 0
					';
		$query .= $this->getSortSQL( $order, $strict );
	...
	function getByUserDateIdAndStatusAndTypeAndPremiumPolicyIdAndOverride($user_date_id, $status, $type, $premium_policy_id, $override = FALSE, $order = NULL) {
		...
		$query = '
					...
					where	a.user_date_id = :user_date_id
						AND a.status_id = :status
						AND a.type_id = :type
						AND a.premium_policy_id = :premium_policy_id
						AND a.override = :override
						AND a.deleted = 0
					';
		$query .= $this->getSortSQL( $order, $strict );
	...
	function getByUserDateIdAndStatusAndTypeAndMealPolicyIdAndOverride($user_date_id, $status, $type, $meal_policy_id, $override = FALSE, $order = NULL) {
		...
		$query = '
					...
					where	a.user_date_id = :user_date_id
						AND a.status_id = :status
						AND a.type_id = :type
						AND a.meal_policy_id = :meal_policy_id
						AND a.override = :override
						AND a.deleted = 0
					';
		$query .= $this->getSortSQL( $order, $strict );
	...
	function getByUserDateIdAndStatusAndTypeAndAbsencePolicyIdAndOverride($user_date_id, $status, $type, $absence_policy_id, $override = FALSE, $order = NULL) {
		...
		$query = '
					...
					where	a.user_date_id = :user_date_id
						AND a.status_id = :status
						AND a.type_id = :type
						AND a.absence_policy_id = :absence_policy_id
						AND a.override = :override
						AND a.deleted = 0
					';
		$query .= $this->getSortSQL( $order, $strict );
	...
	function getByPunchControlId($punch_control_id) {
		...
		$query = '
					select 	*
					from	'. $this->getTable() .'
					where punch_control_id = :punch_control_id
						AND deleted = 0
					';
	...
	function getByUserDateIdAndPunchControlId($user_date_id, $punch_control_id) {
		...
		$query = '
					...
					from	'. $this->getTable() .'
					where	user_date_id = :user_date_id
						AND punch_control_id = :punch_control_id
						AND deleted = 0
					';
	...
	function getDaysWorkedByUserIDAndStartDateAndEndDateAndDayOfWeek( $user_id, $start_date, $end_date, $day_of_week ) {
		...
		$query = '
					...
					where 	a.user_date_id = b.id
						...
						AND b.date_stamp >= :start_date
						AND b.date_stamp <= :end_date
						AND '. $day_of_week_clause .' = :day_of_week
						AND a.status_id = 20
						AND a.total_time > 0
						AND ( a.deleted = 0 AND b.deleted=0 )
				';
	...
	*/
	...
	function getByOverTimePolicyId($over_time_policy_id, $where = NULL, $order = NULL) {
		...
		$query = '
					select 	*
					from	'. $this->getTable() .'
					where	over_time_policy_id = :over_time_policy_id
						AND deleted = 0
					LIMIT 1
				';
		$query .= $this->getWhereSQL( $where );
	...
	function getByMealPolicyId($meal_policy_id, $where = NULL, $order = NULL) {
		...
		$query = '
					select 	*
					from	'. $this->getTable() .'
					where	meal_policy_id = :meal_policy_id
						AND deleted = 0
					LIMIT 1
				';
		$query .= $this->getWhereSQL( $where );
	...
	function getByBreakPolicyId($break_policy_id, $where = NULL, $order = NULL) {
		...
		$query = '
					select 	*
					from	'. $this->getTable() .'
					where	break_policy_id = :break_policy_id
						AND deleted = 0
					LIMIT 1
				';
		$query .= $this->getWhereSQL( $where );
	...
	function getByPremiumTimePolicyId($premium_policy_id, $where = NULL, $order = NULL) {
		...
		$query = '
					select 	*
					from	'. $this->getTable() .'
					where	premium_policy_id = :premium_policy_id
						AND deleted = 0
					LIMIT 1
				';
		$query .= $this->getWhereSQL( $where );
	...
	function getByAbsencePolicyId($absence_policy_id, $where = NULL, $order = NULL) {
		...
		$query = '
					select 	*
					from	'. $this->getTable() .'
					where	absence_policy_id = :absence_policy_id
						AND deleted = 0
					LIMIT 1
				';
		$query .= $this->getWhereSQL( $where );
...
*/
	...
	function getDayReportByCompanyIdAndArrayCriteria($company_id, $filter_data, $limit = NULL, $page = NULL, $where = NULL, $order = NULL) {
		...
		// Centralize date parameter handling
		if (isset($filter_data['start_date']) && trim($filter_data['start_date']) != '') {
			$ph[':sub_start_date'] = Carbon::parse($filter_data['start_date'])->format('Y-m-d');
			$ph[':main_start_date'] = Carbon::parse($filter_data['start_date'])->format('Y-m-d');
		}
		if (isset($filter_data['end_date']) && trim($filter_data['end_date']) != '') {
			$ph[':sub_end_date'] = Carbon::parse($filter_data['end_date'])->format('Y-m-d');
			$ph[':main_end_date'] = Carbon::parse($filter_data['end_date'])->format('Y-m-d');
		}
				...
				( select
					...
					LEFT JOIN ' . $uwf->getTable() . ' as z ON z.id = (select z.id
						...
						where z.user_id = b.user_id
							...
							order by z.effective_date desc limit 1)
					LEFT JOIN (
    select tmp3.id, min(tmp3.min_punch_time_stamp) as min_punch_time_stamp, max(tmp3.max_punch_time_stamp) as max_punch_time_stamp
    from (
        select tmp2_a.id,
            CASE WHEN tmp2_c.status_id = 10 THEN min(tmp2_c.time_stamp) ELSE NULL END as min_punch_time_stamp,
            CASE WHEN tmp2_c.status_id = 20 THEN max(tmp2_c.time_stamp) ELSE NULL END as max_punch_time_stamp
		...
		if (isset($filter_data['start_date']) && trim($filter_data['start_date']) != '') {
			$query .= ' AND tmp2_a.date_stamp >= :sub_start_date';
		}
		if (isset($filter_data['end_date']) && trim($filter_data['end_date']) != '') {
			$query .= ' AND tmp2_a.date_stamp <= :sub_end_date';
		}
		...
		if (isset($filter_data['start_date']) && trim($filter_data['start_date']) != '') {
			$query .= ' AND b.date_stamp >= :main_start_date';
		}
		if (isset($filter_data['end_date']) && trim($filter_data['end_date']) != '') {
			$query .= ' AND b.date_stamp <= :main_end_date';
		}
