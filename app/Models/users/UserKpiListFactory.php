...
class UserKpiListFactory extends UserKpiFactory implements IteratorAggregate {
	...
	function getByUserId($id, $where = NULL, $order = NULL) {
		...
		if ( $order == NULL ) {
			$order = array( 'b.start_date' => 'desc');//ARSP NOTE --> I MODIFIED THIS CODE FOR THUNDER & NEON
			$strict = FALSE;
		} else {
			$strict = TRUE;
		}
	...
	function getUserAccountByCompanyIdAndUserIdAndStartDateAndEndDate($id,$start_date,$end_date, $where = NULL, $order = NULL) {
		if ( $id == '') {
			return FALSE;
		}
