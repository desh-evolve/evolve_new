<?php


namespace App\Models\PayStub;

use App\Models\Core\TTPDF;

class PayStubMyPdfHeaderFooterLandscape extends TTPDF{


    // Page footer
    public function Footer() {

        // Position at 15 mm from bottom
        $this->SetY(-10);
        // Set font
        $this->SetFont('helvetica', 'I', 8);
        // Page number
        $this->Cell(0, 10, 'Page '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'R', 0, '', 0, false, 'T', 'M');
        
        $this->SetFont('','', 6);

	$this->setXY( Misc::AdjustXY(220, 0), Misc::AdjustXY(0, -10) );
        $this->Cell(0, 10, ('Pay Slip Summary Generated by').' '. APPLICATION_NAME.' | ', 0, false, 'L', 0, '', 0, false, 'T', 'M');
        
        $this->SetFont('','', 7 );
        $this->setXY( Misc::AdjustXY(10, 0), Misc::AdjustXY(0, -13) );
        $this->Cell(0, 10, ('...................................'), 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->setXY( Misc::AdjustXY(15, 0), Misc::AdjustXY(0, -10) );
        $this->Cell(0, 10, ('Prepared By'), 0, false, 'L', 0, '', 0, false, 'T', 'M');
        
        
        $this->setXY( Misc::AdjustXY(70, 0), Misc::AdjustXY(0, -13) );
        $this->Cell(0, 10, ('...................................'), 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->setXY( Misc::AdjustXY(75, 0), Misc::AdjustXY(0, -10) );
        $this->Cell(0, 10, ('Checked By'), 0, false, 'L', 0, '', 0, false, 'T', 'M');
        
        
        $this->setXY( Misc::AdjustXY(130, 0), Misc::AdjustXY(0, -13) );
        $this->Cell(0, 10, ('...................................'), 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->setXY( Misc::AdjustXY(135, 0), Misc::AdjustXY(0, -10) );
        $this->Cell(0, 10, ('Approved By'), 0, false, 'L', 0, '', 0, false, 'T', 'M');
    }
    
    
    
    //Page header
    public function Header() {
	
//        var_dump($_SESSION['header_data']['departments']); die;
        $adjust_x = 10;//20
	$adjust_y = 5;//10                
        $border = 0;
                
		//Company Logo                 
		$this->Image( $_SESSION['header_data']['image_path'],Misc::AdjustXY(0, $adjust_x+0 ),Misc::AdjustXY(1, $adjust_y+0 ), 50, 12, '', '', '', FALSE, 300, '', FALSE, FALSE, 0, TRUE);
                

		//Company name
		$this->SetFont('','B',14);

		$this->setXY( Misc::AdjustXY(50, $adjust_x), Misc::AdjustXY(0, $adjust_y) );

		$this->Cell(160,5, $_SESSION['header_data']['company_name'], $border, 0, 'C');

  
        //Company address
		$this->SetFont('','',10);

		$this->setXY( Misc::AdjustXY(50, $adjust_x), Misc::AdjustXY(6, $adjust_y) );

		$this->Cell(160,5,$_SESSION['header_data']['address1'].' '.$_SESSION['header_data']['address2'], $border, 0, 'C');


        //Company city
		$this->setXY( Misc::AdjustXY(50, $adjust_x), Misc::AdjustXY(10, $adjust_y) );

		$this->Cell(160,5,$_SESSION['header_data']['city'].', '.$_SESSION['header_data']['province'].' '. strtoupper($_SESSION['header_data']['postal_code']), $border, 0, 'C');
		
		
		
		if($_SESSION['header_data']['start_date'] != "" && $_SESSION['header_data']['end_date'] != "")
		{
			//Pay Period infomation only text
			$this->SetFont('','',10);
			//from
			$this->setXY( Misc::AdjustXY(125, $adjust_x), Misc::AdjustXY(0, $adjust_y) );
	
			$this->Cell(115,5,('From:').' ', $border, 0, 'R');
			//to
			$this->setXY( Misc::AdjustXY(125, $adjust_x), Misc::AdjustXY(5, $adjust_y) );
	
			$this->Cell(115,5,('To:').' ', $border, 0, 'R');	
					
					
			//Pay Period infomation Values
			$this->SetFont('','B',10);
			//from
			$this->setXY( Misc::AdjustXY(155, $adjust_x), Misc::AdjustXY(0, $adjust_y) );
	
			$this->Cell(105,5, TTDate::getDate('DATE', $_SESSION['header_data']['start_date'] ) , $border, 0, 'R');
	
			//to
			$this->setXY( Misc::AdjustXY(155, $adjust_x), Misc::AdjustXY(5, $adjust_y) );
	
			$this->Cell(105,5, TTDate::getDate('DATE', $_SESSION['header_data']['end_date'] ) , $border, 0, 'R');			
		}
		
		
		
		
		
		if($_SESSION['header_data']['payperiod_string'] != "")//This code use to hide the values
		{
			//Payperiod 
			$this->setLineWidth( 0 );
	
			$this->SetFont('','',10);			
	
			$this->setXY( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(13, $adjust_y) );
	
			$this->Cell(10, 5, 'Pay Period(s) :', $border, 0, 'L');  
					
					
			$this->SetFont('','',8);
			//payperiod value
			$this->setXY( Misc::AdjustXY(24, $adjust_x), Misc::AdjustXY(13, $adjust_y) );
	
			$this->Cell(20,5, $_SESSION['header_data']['payperiod_string'], $border, 0, 'L'); 			
		}
		
                
                /*
                Commented because rosen enterprises filter set  by group not department   
                   */
                
//		if($_SESSION['header_data']['departments'] != "")//This code use to hide the values
//		{
//			//Payperiod 
//			$this->setLineWidth( 0 );
//	
//			$this->SetFont('','',10);			
//	
//			$this->setXY( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(13, $adjust_y) );
//	
//			$this->Cell(10, 15, 'Department(s) :', $border, 0, 'L');  
//					
//					
//			$this->SetFont('','',8);
//			//payperiod value
//			$this->setXY( Misc::AdjustXY(25, $adjust_x), Misc::AdjustXY(13, $adjust_y) );
//	
//			$this->Cell(20,15, $_SESSION['header_data']['departments'], $border, 0, 'L'); 			
//		}
		
                
                //FL ADDED
		if($_SESSION['header_data']['groups'] != "")//This code use to hide the values
		{
			//Payperiod 
			$this->setLineWidth( 0 );
	
			$this->SetFont('','',10);			
	
			$this->setXY( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(13, $adjust_y) );
	
			$this->Cell(10, 15, 'Group(s)        :', $border, 0, 'L');  
					
					
			$this->SetFont('','',8);
			//payperiod value
			$this->setXY( Misc::AdjustXY(25, $adjust_x), Misc::AdjustXY(13, $adjust_y) );
	
			$this->Cell(20,15, $_SESSION['header_data']['groups'], $border, 0, 'L'); 			
		}
                        
                
               
                
                
                                

                
        //CONFIDENTIAL text
		$this->setLineWidth( 0 );

		$this->SetFont('','B',11);				

		$this->setXY( Misc::AdjustXY(184, $adjust_x), Misc::AdjustXY(24, $adjust_y) );//165

		$this->Cell(95, 5, 'CONFIDENTIAL', $border, 0, 'R');     
                
                            
		//Line - start line
		$this->setLineWidth( 1 );

		$this->Line( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(24, $adjust_y), Misc::AdjustXY(284, $adjust_y), Misc::AdjustXY(24, $adjust_y) );//270

          
        // Line text "Pay Slip Summary Report"
		$this->SetFont('','B',11);

		$this->setXY( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(24, $adjust_y) );

		$this->Cell(260, 5, ('Pay Slip Summary Report'), $border, 0, 'C', 0);



		//Line - end line
		$this->setLineWidth( 1 );     
                
        $this->Line( Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(30, $adjust_y), Misc::AdjustXY(284, $adjust_y), Misc::AdjustXY(30, $adjust_y) );//270

        
    }
    
}




?>