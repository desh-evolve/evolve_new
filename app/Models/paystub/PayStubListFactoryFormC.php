...
public function Header() {
    ...
    // Set font for address block
    $this->SetFont('times', '', 11);

    // Safely build the address text
    if (isset($_SESSION['header_data']) && is_array($_SESSION['header_data'])) {
        $txt .= '   ' . ($_SESSION['header_data']['company_name'] ?? 'N/A') . "\n";
        $txt .= '   ' . ($_SESSION['header_data']['address1'] ?? '') . "\n";
        $txt .= '   ' . ($_SESSION['header_data']['address2'] ?? '') . "\n";
        $txt .= '   ' . ($_SESSION['header_data']['city'] ?? '') . ',' . strtoupper($_SESSION['header_data']['postal_code'] ?? '');
    } else {
        $txt .= "   Company information not available\n";
        error_log("Session header_data is not set or not an array in Header(): " . print_r($_SESSION, true));
    }
    ...
    $html = '<table border="1" cellspacing="0" cellpadding="4" width="88%">
        <tr style="text-align:left;">';
    // E.P.F Registration No.
    $html .= '<td width="48%">E.P.F Registration No</td>';
    $html .= '<td style="font-family:times new roman;font-weight:bold;" width="52%">' 
             . (isset($_SESSION['header_data']['epf_registration_no']) ? $_SESSION['header_data']['epf_registration_no'] : 'N/A') 
             . '</td>';
    $html .= '</tr>';

    // Contributions for the Month of
    $html .= '<tr style="text-align:left;">';
    $html .= '<td width="48%">Contributions for the Month of</td>';
    $html .= '<td style="font-family:times new roman;font-weight:bold;" width="52%">' 
             . (isset($_SESSION['header_data']['payperiod_string']) ? $_SESSION['header_data']['payperiod_string'] : 'N/A') 
             . '</td>';
    $html .= '</tr>';

    // Contributions
    $html .= '<tr style="text-align:left;">';
    $html .= '<td width="53%">Contributions</td>';
    // Fix: Replace $_SESSION['header_data'][0] with the correct value or remove if not needed
    $html .= '<td style="font-family:times new roman;font-weight:bold;" width="47%">' 
             . (isset($this->total_contributions) ? number_format($this->total_contributions, 2) : 'N/A') 
             . '</td>';
    $html .= '</tr>';

    // Surcharges
    $html .= '<tr style="text-align:left;">';
    $html .= '<td width="53%">Surcharges</td>';
    $html .= '<td style="font-family:times new roman;font-weight:bold;" width="47%"></td>';
    $html .= '</tr>';

    // Total Remittance
    $html .= '<tr style="text-align:left;">';
    $html .= '<td width="53%">Total Remittance</td>';
    $html .= '<td style="font-family:times new roman;font-weight:bold;" width="47%"></td>';
    $html .= '</tr>';
    ...
    // Table position for employee data
    $this->setXY(Misc::AdjustXY(0, $adjust_x), Misc::AdjustXY(49, $adjust_y));
    $this->SetFont('times', 'B', 10);
