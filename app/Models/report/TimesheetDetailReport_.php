                         ...
                         /*       $elf = TTnew('ExceptionListFactory');
                                ...
                                $ex_obj_arr = $elf->getCurrent()->data;
                                 ...
                                 $status1 = 'WO'; 
                                  ...
                                  $slf = TTnew('ScheduleListFactory');
                                    $slf->getByUserDateId($udlf_obj->getId());
...
//                $html = $html.'<td width = "3%">#</td>';
              ...
              //echo '<br><br><br>';     print_r( $dates ); 
                ...
                foreach($dates as $date){
                    ...
                    $udlf = TTnew('UserDateListFactory');
                    $udlf->getByUserIdAndDate($rows['user_id'],$dateStamp);
                    $udlf_obj = $udlf->getCurrent();
                    $user_date_id = $udlf_obj->getId();
                    ...
                    //echo '<br><br>sys day....'.$day = $date['day'];
                    ...
                    foreach ($plf as $plf_obj) {
                      ...
                      // echo '<br>date........'.$time_stamp_date = date('d', $plf_obj->getTimeStamp());
                        ...
                        if($time_stamp_date!='' && $date['day']!=$time_stamp_date)
                        {
                            if($plf_obj->getStatus()==10)
                            {
                                $new_row[$time_stamp_date]['min_punch_time_stamp'] = date('H:i', $plf_obj->getTimeStamp());
                            }
                            elseif($plf_obj->getStatus()==20)
                            {
                                $new_row[$time_stamp_date]['max_punch_time_stamp'] = date('H:i', $plf_obj->getTimeStamp());
                            }
                        }
                        else
                        {
                            if($plf_obj->getStatus()==10)
                            {
                                $new_row[$date['day']]['min_punch_time_stamp'] = date('H:i', $plf_obj->getTimeStamp());
                            }
                            elseif($plf_obj->getStatus()==20)
                            {
                               $new_row[$date['day']]['max_punch_time_stamp'] = date('H:i', $plf_obj->getTimeStamp());
                            }
                        }
...
//                        $html=  $html.'<td>O</td>';           
                ...
                foreach ($new_row as $key => $key_value) 
                {
                    $html=  $html.'<tr align="center">';        
                    $html=  $html.'<td style="padding:15px;" align="left">'.$key.'</td>';        
                    $html=  $html.'<td>'.$key_value['min_punch_time_stamp'].'</td>';        
                    $html=  $html.'<td>'.$key_value['max_punch_time_stamp'].'</td>';        
                    $html=  $html.'<td>'.$key_value['worked_time'].'</td>';            
                    $html=  $html.'<td>'.$key_value['over_time'].'</td>';            
                    $html=  $html.'<td>'.$key_value['status1'].'</td>';        
                    $html=  $html.'<td>'.$key_value['status2'].'</td>';        
//                        $html=  $html.'<td>O</td>';           
                    $html=  $html.'<td></td>';        
                    $html=  $html.'<td></td>';        
                    $html=  $html.'</tr>';
               ...
               //echo '<br>row_data_day_key'; print_r($row_data_day_key);
