...
//            echo '<pre>'; print_r($data); echo '<pre>'; die;
            ...
            if ( is_array($data) AND count($data) > 0 AND is_array($columns) AND count($columns) > 0 )
            {
                ...
                $html = '<table border="0" cellspacing="0" cellpadding="0" width="60%">
                        <tr style="background-color:#CCCCCC;text-align:center;" >';
//                $html = $html.'<td width = "3%">#</td>';
                $html = $html.'<td> ('.date('Y/M/d',$pay_period_start).' to '.date('Y/M/d',$pay_period_end).') </td>';
        ...
        {  
            ...
            if ( is_array($data) AND count($data) > 0 AND is_array($columns) AND count($columns) > 0 )
            {
                ...
                foreach ($rows as $row){
                    ...
                    $slf_obj_arr = array(); $pc_obj_arr = array();

                    /*echo '<pre><br>-----------------------------------------------------------------<br>'; 
                    echo '<br>---data array---'; 
                    echo '<br>'.$row['employee_number'];
                    ...
                    if(!empty($slf_obj_arr))
                    {
                        ...
                        /*echo '<br>---PunchControl---<br>';
                        print_r($pc_obj_arr);*/

                        if(empty($pc_obj_arr))
                        {

                            /*echo '<br>---Absence user array---'; 
                            echo '<br>'.$row['employee_number'];
                            echo '<br>'.$row['first_name'].' '.$row['last_name'];
                            print_r($row['data']);
...
//                            echo '<pre>'; print_r($pc_obj_arr); die;
        ...
        {  
            ...
            if ( is_array($data) AND count($data) > 0 AND is_array($columns) AND count($columns) > 0 )
            {
                ...
                $_SESSION['header_data'] = array( 
                                                  'payperiod_end_date'   => date('Y-M-d',$pay_period_end),
                                                  'image_path'   => $current_company->getLogoFileName(),
                                                  'company_name' => $current_company->getName(),
                                                  'address1'     => $current_company->getAddress1(),
                                                  'address2'     => $current_company->getAddress2(),
                ...
                $html = $html.'<td align="center" width="10%">Shift In</td>'; 
                $html = $html.'<td align="center" width="10%">Shift Out</td>';
