...
if ( !$permission->Check('report','enabled')
		OR !$permission->Check('report','cform_reports_new') ) {
	$permission->Redirect( FALSE ); //Redirect
}

$smarty->assign('title', TTi18n::gettext($title = 'EPF Payments Details'));  // See index.php
		...
		if ( $action == 'export' AND $filter_data['export_type'] == 'csv' ) {
			...
			if ( isset($rows) AND isset($filter_columns) ) {
				...
				Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                ...
                                $pslf = TTnew( 'PayStubListFactory' );//new code                                
...
//                                echo '<pre>'; print_r($report_data); echo '<pre>';  die;
                            ...
                            if(!$filter_data['bank_code'])
                                echo TTi18n::gettext('Bank Code can not be empty.') ."<br>\n";                                    
                            if(!$filter_data['branch_code'])
                                echo TTi18n::gettext('Branch Code can not be empty.') ."<br>\n";                                    
                            if(!$filter_data['account_number'])
                                echo TTi18n::gettext('Account Number can not be empty.') ."<br>\n";                                    
                            if(!$filter_data['district_office'])
                                echo TTi18n::gettext('District Office Code can not be empty.') ."<br>\n";                                    
                                }
                    }else{
                                echo TTi18n::gettext('Please Select at Least One Pay Period.') ."<br>\n";                                    
                                }
                }  				
	...
	default:
		...
		//$filter_data['export_type_options'] = Misc::prependArray( array( 'csv' => TTi18n::gettext('CSV (Excel)'), 'pdfp' => TTi18n::gettext('PDF (PORTRAIT)'), 'pdfl' => TTi18n::gettext('PDF (LANDSCAPE)'), 'formc' => TTi18n::gettext('Form C (PDF)')), Misc::trimSortPrefix( $psf->getOptions('export_type') ) );
		$filter_data['export_type_options'] = Misc::prependArray( array('formc' => TTi18n::gettext('Form E EPF (PDF)'),'formc_txt' => TTi18n::gettext('Export (TXT)')) ); //ARSP EDIT --> I HIDE THIS CODE ", Misc::trimSortPrefix( $psf->getOptions('export_type') )"  .IT'S USE TO HIDE THE ALL OTHER EXPORT OPTIONS  	
		...
		$smarty->display('report/FormE_payments.tpl');
