...
class DepartmentAttendanceReport extends Controller
{
    ...
    {
        $viewData['title'] = __('Monthly Attendance Per User');
        ...
        $static_columns = [
            '-1000-pay_period' => __('Pay Periods'),
            '-1010-date_stamp' => __('Date'),
            '-1050-min_punch_time_stamp' => __('First In Punch'),
            '-1060-max_punch_time_stamp' => __('Last Out Punch'),
        ];
        ...
        switch ($action) {
            ...
            case 'display_detailed_timesheet':
                ...
                if ($ulf->getRecordCount() > 0) {
                    ...
                    foreach ($udtlf->rs as $udt_obj) {
                        ...
                        if ($column == 'worked_time') {
                            $tmp_rows[$user_id][$pay_period_id][$date_stamp][$column] = ($tmp_rows[$user_id][$pay_period_id][$date_stamp][$column] ?? 0) + (int)$udt_obj->getColumn('total_time');
                            $tmp_rows[$user_id][$pay_period_id][$date_stamp]['actual_time'] = ($tmp_rows[$user_id][$pay_period_id][$date_stamp]['actual_time'] ?? 0) + $udt_obj->getColumn('actual_total_time');

                            $actual_time_diff = bcsub($udt_obj->getColumn('actual_total_time'), $udt_obj->getColumn('total_time'));
                            $tmp_rows[$user_id][$pay_period_id][$date_stamp]['actual_time_diff'] = ($tmp_rows[$user_id][$pay_period_id][$date_stamp]['actual_time_diff'] ?? 0) + $actual_time_diff;

                            $tmp_rows[$user_id][$pay_period_id][$date_stamp]['actual_time_diff_wage'] = isset($user_wage[$user_id]) ?
                            ...
                            if ($status_id == 30 && $type_id == 10) {
                                $tmp_rows[$user_id][$pay_period_id][$date_stamp]['absence_time'] = ($tmp_rows[$user_id][$pay_period_id][$date_stamp]['absence_time'] ?? 0) + $udt_obj->getColumn('total_time');
                            }

                            if ($status_id == 10 && $type_id == 30) {
                                $tmp_rows[$user_id][$pay_period_id][$date_stamp]['over_time'] = ($tmp_rows[$user_id][$pay_period_id][$date_stamp]['over_time'] ?? 0) + $udt_obj->getColumn('total_time');
                                $tmp_rows[$user_id][$pay_period_id][$date_stamp]['over_time_policy_id'] = $policy_id;
                            }

                            $tmp_rows[$user_id][$pay_period_id][$date_stamp][$column] = ($tmp_rows[$user_id][$pay_period_id][$date_stamp][$column] ?? 0) + $udt_obj->getColumn('total_time');

                            if (in_array($action, ['display_timesheet', 'display_detailed_timesheet'])) {
                                $tmp_rows[$user_id][$pay_period_id][$date_stamp]['categorized_time'][$category][$policy_id] =
                                    ($tmp_rows[$user_id][$pay_period_id][$date_stamp]['categorized_time'][$category][$policy_id] ?? 0) +
                                    $udt_obj->getColumn('total_time');
                            }
                        }

                        $tmp_rows[$user_id][$pay_period_id][$date_stamp]['schedule_working'] = $schedule_rows[$pay_period_id][$user_id][$date_stamp]['working'] ?? NULL;
                        $tmp_rows[$user_id][$pay_period_id][$date_stamp]['schedule_absence'] = $schedule_rows[$pay_period_id][$user_id][$date_stamp]['absence'] ?? NULL;
                        $tmp_rows[$user_id][$pay_period_id][$date_stamp]['shedule_start_time'] = $schedule_rows[$pay_period_id][$user_id][$date_stamp]['start_time'] ?? NULL;
                        $tmp_rows[$user_id][$pay_period_id][$date_stamp]['shedule_end_time'] = $schedule_rows[$pay_period_id][$user_id][$date_stamp]['end_time'] ?? NULL;
                ...
                if ($action == 'display_report') {
                    ...
                    $output = $tsdr->DepartmentAttendanceDetailed($rows, $filter_columns, $filter_data, $current_user, $current_company);
                    if (Debug::getVerbosity() < 11) {
                        return response()->streamDownload(function () use ($output) {
                            echo $output;
                        }, 'DepartmentAttendanceReport.pdf', ['Content-Type' => 'application/pdf']);
                    }
                } else {
                    ...
                    return view('report.DepartmentAttendanceReport', $viewData);
                }
                break;
            ...
            default:
                ...
                $filter_data['export_type_options'] = Misc::prependArray([
                    'pdfDepartmentDetailAttendance' => __('Department Attendance Report')
                ]);

                $saved_report_options = $ugdlf->getByUserIdAndScriptArray($current_user->getId(), $_SERVER['SCRIPT_NAME']);
                $viewData['generic_data'] = $generic_data ?? ['saved_report_options' => $saved_report_options];
                $viewData['filter_data'] = $filter_data;
                $viewData['ugdf'] = $ugdf;
                $viewData['hidden_elements'] = $hidden_elements;

                return view('report.DepartmentAttendanceFilter', $viewData);
                break;
        }
    }
}