...
$smarty->assign('title', __($title = 'TimeSheet Detail Report')); // See index.php
					...
					foreach($data_a as $user_id => $data_b ) {
						...
						foreach($data_b as $date_stamp => $data_c ) {
							$sub_rows[$x]['date_stamp'] = $date_stamp;
                                                        ...
                                                        $ch_date->setTimestamp($date_stamp);
                                                        ...
                                                        if($lrlf->getRecordCount()>0){
                                                           ...
                                                           $sub_rows[$x]['leave_type']= $leave_method[$lrf->getLeaveMethod()];
                                                            
                                                        }
                                                        else{
                                                              $sub_rows[$x]['leave_type']='';
                                                        }
		...
		if ( $action == 'display_timesheet' ) {
			if ( isset($rows) ) {
				...
				foreach( $rows as $user_data ) {
					...
					$pdf->SetFont('','B',32);
					$pdf->Cell(200,15, __('Employee Time Card') , $border, 0, 'C');
					$pdf->Ln();
					$pdf->SetFont('','B',12);
					...
					$pdf->SetFont('','B',12);
					$pdf->Cell(40,5, $user_data['first_name'] .' '. $user_data['last_name'] .' (#'. $user_data['employee_number'] .')', $border, 0, 'L');
					$pdf->Ln();
					...
					$pdf->Ln();
                                        ...
                                        $pdf->SetFont('','',12);
					$pdf->Cell(140,5, __('Pay Period:') , $border, 0, 'R');
					$pdf->SetFont('','',12);
					...
					$column_widths = array(
										...
										'date_stamp' => 20,
										'dow' => 11,
										'min_punch_time_stamp' => 29,
										'max_punch_time_stamp' => 29,
										'worked_time' => 29,
										'regular_time' => 29,
										'over_time' => 24,
										'paid_time' => 24,
...
$pdf->Ln();
								...
								$pdf->MultiCell( $column_widths['min_punch_time_stamp'], $line_h, __('In') , 1, 'C', 1, 0);
								$pdf->MultiCell( $column_widths['max_punch_time_stamp'], $line_h, __('Out') , 1, 'C', 1, 0);
								...
								//$pdf->MultiCell( $column_widths['paid_time'], $line_h, __('Paid Time') , 1, 'C', 1, 0);
                                                                $pdf->MultiCell( $column_widths['absence_time'], $line_h, __('Remarks') , 1, 'C', 1, 0);
						...
						foreach( $user_data['data'] as $data) {
							...
							if ( $data['date_stamp'] !== '' ) {
								...
								//$pdf->Cell( $column_widths['absence_time'], 6, TTDate::getTimeUnit( $data['absence_time'] ), 1, 0, 'C', 1);
                                                                $pdf->Cell( $column_widths['absence_time'], 6, $data['leave_type'], 1, 0, 'C', 1);
								$pdf->Ln();
$k++;
								unset($hday_obj_arr);
							}
...
//                                 echo '<pre>'; print_r($rows[0]['data']); echo '<pre>';  die;
                                 Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                
                                $tssr= new TimesheetDetailReport();//new code                                
                                $output = $tssr->OTDetailReport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               
...
//                                 echo '<pre>'; print_r($rows[0]['data']); echo '<pre>';  die;
                                ...
                                $tssr= new TimesheetDetailReport();//new code                                
                                $output = $tssr->EmployeeTimeSheet($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               
