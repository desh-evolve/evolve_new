...
$smarty->assign('title', TTi18n::gettext($title = 'Form C Six Months Report'));  // See index.php
	...
	case 'display_report':
		...
		if ( $ulf->getRecordCount() > 0 ) {
			...
			if ( ( ( isset($filter_data['transaction_start_date']) AND isset($filter_data['transaction_end_date']) ) OR isset($filter_data['pay_period_ids']) )
				...
				//ARSP EDIT-->Add New code for $filter_data['export_type'] != 'pdf'
				 elseif ( $action == 'export' AND $filter_data['export_type'] != 'csv' AND $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl' AND $filter_data['export_type'] != 'formcsixmonth')//ARSP EDIT --> ADD NEW CODE for $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl'
				  {
		...
		if ( $action == 'export' AND $filter_data['export_type'] == 'csv' ) {
			if ( isset($rows) AND isset($filter_columns) ) {
                ...
                /*
                 * SIX MONTHS FORM C REPORT 
                 * THIS CODE ADDED BY ME                 * 
                 * PAGE ORIENTATION IS LANDSCAPE
                 */      
                
                //PAGE ORIENTATION IS LANDSCAPE 
                else if ( $action == 'export' AND $filter_data['export_type'] == 'formcsixmonth' ) {             

                    if(count($filter_data['pay_period_ids']) == 6 )
                    {
                        $set1 = 0;
                        $set2 = 0;
                        ...
                        {
                            $pay_period_month[] = substr($pay_period_options[$id],17,2);// get only the month 2 digit
                            switch ($pay_period_month[$x++])
                            {
                                case '1' :$set1++;break;
                                case '01':$set1++;break;
                                case '2' :$set1++;break;
                            ...
                            $pay_period_year[]  = substr($pay_period_options[$id],20,4);          
                        }
                        
                        
                        if ( isset($rows) && $filter_data['include_user_ids']) {                      
                            
                            if( ( (count(array_unique($pay_period_year)) == 1) && ($set1 == 6) ) || ( (count(array_unique($pay_period_year)) == 1) && ($set2 == 6) ) ) //count(array_unique($pay_period_year)) --> check all the elements of this array are same or not
                            {
                                if($set1 == 6)
                                {
                                    $set = 1;
                                ...
                                Debug::Text('Exporting as Six Months Form C PDF', __FILE__, __LINE__, __METHOD__,10);
//                                echo '<pre>';   print_r($rows); echo '<pre>'; die;
                               
                                $pslf = TTnew( 'PayStubListFactory' );//new code                                
                                $output = $pslf->FormCSixMonth($rows, $filter_data['include_user_ids'], $current_user, $current_company, $pay_period_year[0], $set);//new code  
                                

                                if ( Debug::getVerbosity() < 11 ) {
                                    
                                    Misc::FileDownloadHeader('form_c_six_month.pdf', 'application/pdf', strlen($output));
                                    echo $output;
                                    exit;                           
                                        }
                            }
                            ...
                            {
                                echo TTi18n::gettext('Invalid Payperiod Group!! Please Select JANUARY - JUNE or JULY - DECEMBER Months Group of Same Year Pay Periods.') ."<br>\n";        
                            }                            
                        }else {
                        ...
                        echo TTi18n::gettext('Pleasae Select Six months Pay Period. JANUARY - JUNE or JULY - DECMBER ') ."<br>\n"; 
                        }                    
                }  				
	...
	default:
		...
		$filter_data['export_type_options'] = Misc::prependArray( array('formcsixmonth' => TTi18n::gettext('PDF')) ); //ARSP EDIT --> I HIDE THIS CODE ", Misc::trimSortPrefix( $psf->getOptions('export_type') )"  .IT'S USE TO HIDE THE ALL OTHER EXPORT OPTIONS  	
		...
		$smarty->display('report/FormCSixMonth.tpl');
