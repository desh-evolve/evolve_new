...
class FormEPayments extends Controller
{
	...
	public function index()
	{
		...
		$viewData['title'] = 'EPF Payments Details';
		...
		switch ($action) {
			...
			case 'display_report':
				...
				if ($ulf->getRecordCount() > 0) {
					...
					if (((isset($filter_data['transaction_start_date']) and isset($filter_data['transaction_end_date'])) or isset($filter_data['pay_period_ids']))
						...
						elseif ($action == 'export' and $filter_data['export_type'] != 'csv' and $filter_data['export_type'] != 'pdfp' and $filter_data['export_type'] != 'pdfl' and $filter_data['export_type'] != 'formc' and $filter_data['export_type'] != 'formc_txt') //ARSP EDIT --> ADD NEW CODE for $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl'
						{
							...
							//					var_dump($filter_data['pay_period_ids']);die;
							//                                        var_dump($raw_rows[1]);die;    

							/* FL ADDED FOR CALCULATE TOTAL AMOUNT FOR PAYMENTS FOR SELECTED PAYPERIODS OF ALL USERS*/

							$pslf = new PayStubListFactory();
							$clf = new CompanyListFactory();
							$clf->getById($current_company->getId());
							$company_det = $clf->getCurrent();
							//                                        var_dump($filter_data); die;
							$report_data = [];
							// foreach ($raw_rows as $user_all_paystubs) {
								
							// 	foreach ($user_all_paystubs as $user_single_paystubs) {
							// 		$tot_countribution = 0;
							// 		$payperiod_id = $user_single_paystubs['pay_period_id'];
							// 		if (isset($user_single_paystubs['pay_stub_entry_name']['9']) && isset($user_single_paystubs['pay_stub_entry_name']['11'])) {
							// 			$tot_countribution = doubleval($user_single_paystubs['pay_stub_entry_name']['9']) + doubleval($user_single_paystubs['pay_stub_entry_name']['11']);
							// 		}
							// 		// $tot_countribution = doubleval($user_single_paystubs['pay_stub_entry_name']['9']) + doubleval($user_single_paystubs['pay_stub_entry_name']['11']);
							// 		$report_data[$payperiod_id]['total_countribution'] =  $report_data[$payperiod_id]['total_countribution']  + $tot_countribution;

							// 		$count = $pslf->getMembersForPayPeriod($payperiod_id);
							// 		$report_data[$payperiod_id]['member_count'] =  $pslf->getRecordCount();;
							// 		$report_data[$payperiod_id]['payperiod_id'] = $payperiod_id;
							// 		$report_data[$payperiod_id]['pay_stub_transaction_date'] = date('Y-m-d', $user_single_paystubs['pay_stub_transaction_date']);
							// 		$report_data[$payperiod_id]['zone'] = $filter_data['zone'];
							// 		$report_data[$payperiod_id]['district_office'] = $filter_data['district_office'];
							// 		$report_data[$payperiod_id]['payment_mode'] = $filter_data['pay_mode'];
							// 		$report_data[$payperiod_id]['payment_ref'] = $filter_data['bank_code'] . $filter_data['branch_code'] . $filter_data['account_number'];
							// 		$report_data[$payperiod_id]['employer_num'] = $company_det->getEpfNo();

							// 		$payperiod_string = "";

							// 		if ($payperiod_id > 0) {
							// 			$sub_string = substr($pay_period_options[$payperiod_id], 14); // this is the pay period format string(24) "26/03/2013 -> 25/04/2013"
							// 			$replace_string = str_replace("/", "-", $sub_string); // replace string '26/03/2013' to '26-03-2013'

							// 			// $date = new DateTime($replace_string);  
							// 			// print_r(substr($replace_string,0,10));
							// 			$date = new DateTime(substr($replace_string, 0, 11));
							// 			//$date = DateTime::createFromFormat('Y-m-d h:i:s', $replace_string);                              
							// 			$payperiod_string = $date->format('Y-m-d h:i:s'); //only get Month Year Format eg:- April 2013
							// 		}
							// 		$report_data[$payperiod_id]['payperiod_string'] = $payperiod_string;
							// 	}
							// }
							...
							foreach ($raw_rows as $user_all_paystubs) {
								foreach ($user_all_paystubs as $user_single_paystubs) {
									...
									// Initialize report_data for this payperiod_id if not set
									if (!isset($report_data[$payperiod_id])) {
										$report_data[$payperiod_id] = [
											'total_countribution' => 0,
											'member_count' => 0,
											'payperiod_id' => $payperiod_id,
											'payperiod_string' => '',
											'pay_stub_transaction_date' => '',
											'zone' => $filter_data['zone'] ?? '',
											'district_office' => $filter_data['district_office'] ?? '',
											'payment_mode' => $filter_data['pay_mode'] ?? '',
											'payment_ref' => ($filter_data['bank_code'] ?? '') . ($filter_data['branch_code'] ?? '') . ($filter_data['account_number'] ?? ''),
											'employer_num' => $company_det->getEpfNo() ?? ''
										];
									}
									...
									$report_data[$payperiod_id]['total_countribution'] += $tot_countribution;

									// Get member count for pay period
									// dd($payperiod_id);
									...
									// Set pay stub transaction date
									$report_data[$payperiod_id]['pay_stub_transaction_date'] = date('Y-m-d', $user_single_paystubs['pay_stub_transaction_date'] ?? time());

									// Pay period string processing
									$payperiod_string = '';
									if ($payperiod_id > 0 && isset($pay_period_options[$payperiod_id])) {
										$sub_string = substr($pay_period_options[$payperiod_id], 14); // Assumes format "26/03/2013 -> 25/04/2013"
										$replace_string = str_replace("/", "-", $sub_string); // Replace '/' with '-'

										try {
											// Extract first 10 characters (e.g., "25-04-2013")
											$date = DateTime::createFromFormat('d-m-Y', substr($replace_string, 0, 10));
											if ($date !== false) {
												$payperiod_string = $date->format('Y-m-d H:i:s');
											}
										} catch (Exception $e) {
							...
							/* END FL ADDED FOR CALCULATE TOTAL AMOUNT FOR PAYMENTS FOR SELECTED PAYPERIODS OF ALL USERS*/
							if (isset($raw_rows)) {
								$ulf = new UserListFactory();
				...
				/*
                 * FL ADDED NEW CODE HERE...
                 * ARSP EDIT --> ADD NEW CODE FOR Export PDF REPORT OF EPF PAYMENTS
                 ...
                 * ARSP EDIT --> ADD NEW CODE CREATE FORM C PAYMENTS
                 * PAGE ORIENTATION IS PORTRAIT
                 */
				//PAGE ORIENTATION IS PORTRAIT 
				else if ($action == 'export' and ($filter_data['export_type'] == 'formc' || $filter_data['export_type'] == 'formc_txt')) {
					...
					if (count($filter_data['pay_period_ids']) > 0) {
						...
						foreach ($filter_data['pay_period_ids'] as $id) {
							if ($id > 0) {
								...
								// $date = new DateTime($replace_string);
								$date = new DateTime(substr($replace_string, 0, 11));
								$payperiod_string .= $date->format('Ym') . ', '; //only get Month Year Format eg:- April 2013
							} else {
								$payperiod_string = "ALL  "; //if selected pay period is "All"  then only print "ALL" do not need to print all the pay period values. put 2 space after the "ALL"
							}
						}
						...
						if (isset($report_data) && $filter_data['include_user_ids'] && $filter_data['bank_code'] && $filter_data['branch_code'] && $filter_data['account_number'] && $filter_data['district_office']) {

							Debug::Text('Exporting as Form C PDF', __FILE__, __LINE__, __METHOD__, 10);
							//                                echo '<pre>'; print_r($report_data); echo '<pre>';  die;

							$pslf = new PayStubListFactory(); //new code                                
							$output = $pslf->FormE_payments($report_data, $filter_data['include_user_ids'], $current_user, $current_company, $payperiod_string, $branch_id_only, $filter_data['export_type'], $filter_data['submission_number']); //new code    
							...
							if (!$filter_data['include_user_ids'])
								echo __('Please Select at Least One Employee.') . "<br>\n";
							if (!$filter_data['bank_code'])
								echo __('Bank Code can not be empty.') . "<br>\n";
							if (!$filter_data['branch_code'])
								echo __('Branch Code can not be empty.') . "<br>\n";
							if (!$filter_data['account_number'])
								echo __('Account Number can not be empty.') . "<br>\n";
							if (!$filter_data['district_office'])
								echo __('District Office Code can not be empty.') . "<br>\n";
						}
					} else {
			...
			default:
				...
				$balf = new BankAccountListFactory();
				$company_bank = $balf->getCompanyAccountByCompanyId($current_company->getId());
				$bnk1 = $company_bank->getCurrent();
				$filter_data['branch_code1'] = $bnk1->getBankCode();
				$filter_data['account1'] = $bnk1->getAccount();
				$filter_data['date_payments1'] = date('d/m/Y');
				...
				return view('report/FormE_payments', $viewData);
