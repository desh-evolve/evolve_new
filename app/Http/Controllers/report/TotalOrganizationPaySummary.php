...
if ( !$permission->Check('report','enabled')
		OR !$permission->Check('report','tot_org_summary') ) {
	$permission->Redirect( FALSE ); //Redirect
}

$smarty->assign('title', TTi18n::gettext($title = 'Total Organization Pay Summary'));  // See index.php
...
/*				if ( $action == 'view_pay_stubs' ) {
				...
				/*
				 * ARSP EDIT --> I ADDE THIS CODE, I ADDED NEW CODE FOR EXPORT TOTAL ORGANIZATION PAY SUMMARY FOR CHILD FUND
                            	 */   
					...
					if ( $pslf->getRecordCount() > 0 ) {
						if ( !isset($filter_data['hide_employer_rows']) ) {
							...
							$filter_data['hide_employer_rows'] = FALSE;
                                                ...
                                                /*
                                                 * ARSP EDIT --> I ADD THIS CODE FOR GET THE PAY PERIOD VALUES
                                                 */
                                                $payperiod_string= "";   
                                                ...
                                                if (  count($filter_data['include_user_ids']) > 0) {       

						$output = $pslf->TotalOrganizationPaySummary( $pslf, (bool)$filter_data['hide_employer_rows'], $current_company, $payperiod_string ,count($filter_data['include_user_ids']) );
                                                
						if ( Debug::getVerbosity() < 11 ) {
							Misc::FileDownloadHeader('total_organization_summary.pdf', 'application/pdf', strlen($output));
							echo $output;
							exit;
						}
				...
				//ARSP EDIT-->Add New code for $filter_data['export_type'] != 'pdf'
				 elseif ( $action == 'export' AND $filter_data['export_type'] != 'csv' AND $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl' AND $filter_data['export_type'] != 'total_organization_pay')//ARSP EDIT --> ADD NEW CODE for $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl'
				  {
...
//                                exit();
	...
	default:
		...
		//$filter_data['export_type_options'] = Misc::prependArray( array('formc' => TTi18n::gettext('Form C (PDF)')) ); //ARSP EDIT --> I HIDE THIS CODE ", Misc::trimSortPrefix( $psf->getOptions('export_type') )"  .IT'S USE TO HIDE THE ALL OTHER EXPORT OPTIONS  	
                $filter_data['export_type_options'] = Misc::prependArray( array('total_organization_pay' => TTi18n::gettext('Total Organization Pay Summary (PDF)')) ); //ARSP EDIT --> I HIDE THIS CODE ", Misc::trimSortPrefix( $psf->getOptions('export_type') )"  .IT'S USE TO HIDE THE ALL OTHER EXPORT OPTIONS              
		...
		$smarty->display('report/TotalOrganizationPaySummary.tpl');
