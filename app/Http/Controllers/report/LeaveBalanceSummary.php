...
$smarty->assign('title', __($title = 'Leave Balance Summary Report')); // See index.php
...
switch ($action) {
	...
	case 'display_report':
                    ...
                    foreach( $ulf as $u_obj ) { 
                        ...
                        //foreach ($filter_data['column_ids'] as $column_abs ){
                            ...
                            if($colAbs_arr[0] == 'absence_policy' && $column_abs[1] != ''){
                                ...
                                $rows[$x][$column_abs] =  $absence_leave[1]->getShortCode().
                                ' - '.$absence_bal['full_day'].
                                ' | '.$absence_leave[2]->getShortCode().
                                ' - '.$absence_bal['half_day'].
                                ' | '.$absence_leave[3]->getShortCode().
                                ' - '.$absence_bal['short_leave'];
                            ...
                            $rows[$x]['full_name'] = $user_obj->getFirstName().' '.$user_obj->getLastName();//$user_obj->getFullName(TRUE);
       ...
       // echo '<pre>'; print_r($rows); die;
        ...
        foreach ($columns as $column_key => $column_key_vl){
