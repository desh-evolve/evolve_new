...
$smarty->assign('title', TTi18n::gettext($title = 'Lieu Leave And Duty to Lieu Leave Report')); // See index.php
						...
						if ($status_id == 30 AND $type_id == 10) {
							if ( isset($tmp_rows[$pay_period_id][$user_id][$date_stamp]['absence_time']) ) {
								$tmp_rows[$pay_period_id][$user_id][$date_stamp]['absence_time'] += $udt_obj->getColumn('total_time');
                                                                $tmp_rows[$pay_period_id][$user_id][$date_stamp]['absence_type'] = $udt_obj->getColumn('absence_policy_id');
							} else {
								$tmp_rows[$pay_period_id][$user_id][$date_stamp]['absence_time'] = $udt_obj->getColumn('total_time');
                                                                $tmp_rows[$pay_period_id][$user_id][$date_stamp]['absence_type'] = $udt_obj->getColumn('absence_policy_id');
							}
			...
			if ( isset($rows) AND isset($filter_columns) ) {
				...
				echo $data;
                            ...
                            if( $filter_data['export_type'] == 'pdfMonthlyDetailLate'){
                                ...
                                $output = $tsdr->LieuLeaveAndDutyLeaveReport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               
...
*/
		...
		$filter_data['sort_direction_options'] = Misc::getSortDirectionArray();
                ...
                $filter_data['export_type_options'] = Misc::prependArray( array( 'pdfMonthlyDetailLate' => TTi18n::gettext('Lieu Leave and Duty to Lieu Leave Report')) );
