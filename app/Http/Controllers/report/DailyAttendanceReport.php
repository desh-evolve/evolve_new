    ...
    public function index()
    {
        ...
        if (isset($filter_data['print_timesheet']) && $filter_data['print_timesheet'] >= 1) {
            ...
            if (
                ...
                !$permission->Check('report', 'view_timesheet_summary')
            ) {
                $permission->Redirect(false);
            }
        }
        ...
        $filter_data['start_date'] = isset($filter_data['start_date']) ? $filter_data['start_date'] : [];
        $filter_data['end_date'] = isset($filter_data['end_date']) ? $filter_data['end_date'] : [];

        $static_columns = [
            '-1000-date_stamp' => TTi18n::gettext('Date'),
            '-1050-min_punch_time_stamp' => TTi18n::gettext('First In Punch'),
            '-1060-max_punch_time_stamp' => TTi18n::gettext('Last Out Punch'),
        ];
...
//                             }
                            ...
                            if ($ulf->getRecordCount() > 0) {
                                ...
                                if (isset($filter_data['pay_period_ids'])) {
                                    ...
                                    unset($tmp_filter_pay_period_ids, $filter_pay_period_id);
                                }
                            ...
                            if ($action == 'display_timesheet') {
                                ...
                                if (isset($output) && $output !== false && Debug::getVerbosity() < 11) {
                                ...
                                if (isset($rows)) {
                                    ...
                                    // Preprocess $data to convert "--" to 0 for calculations
                                                    ...
                                                    if ($data['over_time'] > 0 && isset($data['categorized_time']['over_time_policy'])) {
                                                        ...
                                                        $over_time = ($data['over_time'] === "--") ? 0 : $data['over_time'];
                                                    ...
                                                    if ($data['absence_time'] > 0 && isset($data['categorized_time']['absence_policy'])) {
                                                        ...
                                                        $absence_time = ($data['absence_time'] === "--") ? 0 : $data['absence_time'];
                                        ...
                                        if (isset($totals) && is_array($totals)) {
                                            ...
                                            $pdf->Cell($column_widths['worked_time'], 6, ($totals['worked_time'] === "--") ? "--" : TTDate::getTimeUnit($totals['worked_time']), 1, 0, 'C', 1);
                                            // $pdf->Cell($column_widths['worked_time'], 6, TTDate::getTimeUnit($totals['worked_time']), 'T', 0, 'C', 0);
                                            $pdf->Cell($column_widths['paid_time'], 6, TTDate::getTimeUnit($totals['paid_time']), 'T', 0, 'C', 0);
                                            $pdf->Cell($column_widths['regular_time'], 6, TTDate::getTimeUnit($totals['regular_time']), 'T', 0, 'C', 0);
                                            $pdf->Cell($column_widths['over_time'], 6, TTDate::getTimeUnit($totals['over_time']), 'T', 0, 'C', 0);
                                ...
                                if (isset($rows) && isset($filter_columns)) {
                                    ...
                                    if ($filter_data['export_type'] == 'pdfDailyDetailAttendance') {
                                        Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__, 10);

                                        $tsdr = new TimesheetDetailReport();
                                        $output = $tsdr->DailyAttendanceDetailed($rows, $filter_columns, $filter_data, $current_user, $current_company);
                                    ...
                                    if ($filter_data['export_type'] == 'pdfMonthlyDetailLate') {
                                        //                                 echo '<pre>'; print_r($rows[0]); echo '<pre>';  die;
                                        Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__, 10);

                                        $tsdr = new TimesheetDetailReport; //new code                                
                                        $output = $tsdr->MonthlyLateDetailed($rows, $filter_columns, $filter_data, $current_user, $current_company); //new code                               

                                        if (Debug::getVerbosity() < 11) {
                                            Misc::FileDownloadHeader('MonthlyDetailLate.pdf', 'application/pdf', strlen($output));
                                            echo $output;
                                            exit;
                                        }
                                    }

                                    //FL ADDED FOR GET DAILY LATE REPORT 20160518
                                    if ($filter_data['export_type'] == 'pdfDailyLate') {
                                        Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__, 10);

                                        $tssr = new TimesheetDetailReport; //new code                                
                                        $output = $tssr->DailyLateReport($rows, $filter_columns, $filter_data, $current_user, $current_company); //new code                               

                                        if (Debug::getVerbosity() < 11) {
                                            Misc::FileDownloadHeader('DailyLateReport.pdf', 'application/pdf', strlen($output));
                                            echo $output;
                                            exit;
                                        }
                                    }
                        ...
                        default:
                            ...
                            if ($action == 'load') {
                                ...
                                if ($ugdlf->getRecordCount() > 0) {
                                    ...
                                    $filter_data['column_ids'] = array_merge(
                            ...
                            $filter_data = Misc::preSetArrayValues($filter_data, array(
                                'include_user_ids',
                                'exclude_user_ids',
                                'user_status_ids',
                                'group_ids',
                            ...
                            $filter_data['export_type_options'] = Misc::prependArray(array(
