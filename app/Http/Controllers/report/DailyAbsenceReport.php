...
class DailyAbsenceReport extends Controller
{
	...
	public function index()
	{
		...
		// $smarty->assign('title', ___($title = 'Head Count & Absence Report')); // See index.php
...
*/
		...
		switch ($action) {
			...
			case 'display_detailed_timesheet':
				...
				if ($action == 'display_timesheet') {
					...
					if (isset($rows) and isset($filter_columns)) {
						...
						if ($filter_data['export_type'] == 'pdfDailyAbsence') {

							Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__, 10);
							$tssr = new TimesheetDetailReport();
							//print_r($rows);
							$output = $tssr->DailyAbsenceReport($rows, $filter_columns, $filter_data, $current_user, $current_company);



							if (Debug::getVerbosity() < 11) {
								Misc::FileDownloadHeader('DailyAbsenceReport.pdf', 'application/pdf', strlen($output));
								echo $output;
								exit;
							}
						}
			...
			default:
				...
				$filter_data['export_type_options'] = Misc::prependArray(array('pdfDailyAbsence' => __('Daily Absence')));
...
*/
				...
				$viewData['ugdf'] = $ugdf;
				return view('report/DailyAbsenceReport', $viewData);
				// $smarty->display('report/DailyAbsenceReport.tpl');
