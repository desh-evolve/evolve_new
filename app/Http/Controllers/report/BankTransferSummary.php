...
$smarty->assign('title', TTi18n::gettext($title = 'Bank Transfer Summary'));  // See index.php
	...
	case 'display_report':
		...
		if ( $ulf->getRecordCount() > 0 ) {
			...
			if ( ( ( isset($filter_data['transaction_start_date']) AND isset($filter_data['transaction_end_date']) ) OR isset($filter_data['pay_period_ids']) )
					...
					/*
					 * ARSP EDIT --> I HIDE THIS CODE, I ADDED NEW CODE FOR EXPORT TOTAL ORGANIZATION PAY SUMMARY FOR CHILD FUND
					 */
				...
				//ARSP EDIT-->Add New code for $filter_data['export_type'] != 'pdf'
				 elseif ( $action == 'export' AND $filter_data['export_type'] != 'csv' AND $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl' AND $filter_data['export_type'] != 'bank_transfer')//ARSP EDIT --> ADD NEW CODE for $filter_data['export_type'] != 'pdfp' AND $filter_data['export_type'] != 'pdfl'
				  {
		...
		if ( $action == 'export' AND $filter_data['export_type'] == 'csv' ) {
			...
			if ( isset($rows) AND isset($filter_columns) ) {
                 ...
                 * ARSP EDIT --> ADD NEW CODE CREATE BANK TRANSFER SUMMARY FOR CHILD FUND
                 * PAGE ORIENTATION IS PORTRAIT
                 */      
                //PAGE ORIENTATION IS PORTRAIT 
/*                else if ( $action == 'export' AND $filter_data['export_type'] == 'bank_transfer' ) {
...
//                                exit();
			...
			if ( isset($rows) && $filter_data['include_user_ids']) {    
				...
				Debug::Text('Exporting as Total Organization Pay Summary PDF', __FILE__, __LINE__, __METHOD__,10);
                                
                                $pslf = TTnew( 'PayStubListFactory' );//new code                                
                                $output = $pslf->BankTransfer($rows, $filter_data['include_user_ids'], $current_user, $current_company, $payperiod_string);//new code    
...
//                    print_r($filter_data);
//                    echo "<p/>";
//                    echo "Pay Period Id - ".count($filter_data['pay_period_ids'])."<p/>";
//                    print_r($filter_data['pay_period_ids']);
//                    exit();
...
//                                exit();
                        $payperiod_string = substr_replace($payperiod_string ,"",-1);//remove the last space
                        $payperiod_string = substr_replace($payperiod_string ,"",-1);//remove the last comma(',')							
...
//                                $pslft = TTnew( 'PayStubListFactoryTest' );//new code                                
//                                $output = $pslft->testfunction();//new code                                     
                               
                                                                
                                if ( Debug::getVerbosity() < 11 ) {                                    
                                    Misc::FileDownloadHeader('banktransfer.pdf', 'application/pdf', strlen($output));
                                    echo $output;
                                    exit;                           
                                }                        
                        }else {
	...
	default:
		...
		//$filter_data['export_type_options'] = Misc::prependArray( array('formc' => TTi18n::gettext('Form C (PDF)')) ); //ARSP EDIT --> I HIDE THIS CODE ", Misc::trimSortPrefix( $psf->getOptions('export_type') )"  .IT'S USE TO HIDE THE ALL OTHER EXPORT OPTIONS  	
                $filter_data['export_type_options'] = Misc::prependArray( array('bank_transfer' => TTi18n::gettext('Bank Transfer Summary (PDF)'),'csv' => TTi18n::gettext('Export CVS (Excel)')) ); //ARSP EDIT --> I HIDE THIS CODE ", Misc::trimSortPrefix( $psf->getOptions('export_type') )"  .IT'S USE TO HIDE THE ALL OTHER EXPORT OPTIONS              
