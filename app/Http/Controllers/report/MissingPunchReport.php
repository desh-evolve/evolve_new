...
class MissingPunchReport extends Controller
{
	...
	public function index()
	{
		...
		$viewData['title'] = 'Missing Punch Report';
...
/*
        ...
        /*else{
        	$datetime1 = new DateTime(date('Y-m-d',$filter_data['start_date']));
			$datetime2 = new DateTime(date('Y-m-d',$filter_data['end_date']));
			$difference = $datetime1->diff($datetime2);
			if($difference->d > 14)
			{
				echo 'Error! Maximum Date difference Should be 14 Days.'; 
            	exit();
			}
        }*/
...
//echo '<pre>'.print_r($tmp_rows).'<pre>';die;
			if ( isset($tmp_rows) ) {
				...
				foreach($tmp_rows as $pay_period_id => $data_a ) {
					...
					foreach($data_a as $user_id => $data_b ) {
						...
						if ( isset($sub_rows) ) {
							...
							//echo '<pre>---------------<br><br><br>'.$i.'<br>'; print_r($tmp_sub_rows); 
							...
							foreach($tmp_sub_rows as $sub_row ) {
								...
								if( ($sub_row['min_punch_time_stamp']!='' AND  $sub_row['max_punch_time_stamp']=='') OR ($sub_row['min_punch_time_stamp']=='' AND  $sub_row['max_punch_time_stamp']!='') )
								{
		...
		if ( $action == 'export' ) {
	      ...
	      if ( isset($rows) AND isset($filter_columns) ) {
                            ...
                            if( $filter_data['export_type'] == 'pdfMissingPunch'){
                                ...
                                $output = $tssr->MissingPunchReport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               

                                if ( Debug::getVerbosity() < 11 ) {                                    
                                    Misc::FileDownloadHeader('MissingPunchReport.pdf', 'application/pdf', strlen($output));
                                    echo $output;
                                    exit;                           
                                }  
                            }
                            if( $filter_data['export_type'] == 'excelMissingPunch'){
                                
                                $tssr= new TimesheetDetailReport();
                                $output = $tssr->MissingPunchReportExcelExport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               
			...
			$viewData['rows'] = $rows;
			return view('report/MissingPunchReport', $viewData);
		}
...
*/
		...
		$filter_data['sort_direction_options'] = Misc::getSortDirectionArray();
				...
				$viewData['hidden_elements'] = $hidden_elements; // See index.php
                
                //FL ADDED FOR EXPORT TYPE
                $filter_data['export_type_options'] = Misc::prependArray( array(  'pdfMissingPunch' => __('Missing Punch Report'),'excelMissingPunch'=>__('Missing Punch Ecel')) );
