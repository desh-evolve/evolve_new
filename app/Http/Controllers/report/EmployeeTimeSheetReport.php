...
$smarty->assign('title', TTi18n::gettext($title = 'Attendance Report')); // See index.php
...
*/
		...
		if ( $ulf->getRecordCount() > 0 ) {
			...
			//echo '<pre>'; print_r($schedule_rows); echo'<pre>';die;
			...
			foreach ($udtlf as $udt_obj ) {
				...
				if ( $column == 'worked_time' ) {
					...
					unset($actual_time_diff);
                                ...
                                //  echo '<pre>'; print_r($tmp_rows[$pay_period_id][$user_id]); echo '<pre>'.$column; 
                                    
                                   // echo  $udt_obj->getColumn('total_time');
					...
					foreach($data_a as $user_id => $data_b ) {
						...
						if ( isset($sub_rows) ) {
							...
							$sub_rows = Sort::Multisort($tmp_sub_rows, Misc::trimSortPrefix($filter_data['primary_sort']), Misc::trimSortPrefix($filter_data['secondary_sort']), $filter_data['primary_sort_dir'], $filter_data['secondary_sort_dir']);
                                                     ...
                                                     // echo '<pre>'; print_r($sub_rows); echo '<pre>'; die;    
...
//                                 echo '<pre>'; print_r($rows[0]['data']); echo '<pre>';  die;
                            ...
                            if ( $action == 'export' AND $filter_data['export_type'] == 'csv_format' ) {
                                    if ( isset($rows) AND isset($filter_columns) ) {
                                            Debug::Text('Exporting as CSV', __FILE__, __LINE__, __METHOD__,10);
...
*/
		...
		$filter_data['sort_direction_options'] = Misc::getSortDirectionArray();
                ...
                $filter_data['export_type_options'] = Misc::prependArray( array( 'mothlyTimesheet' => TTi18n::gettext('Employee Time Sheet (PDF)') , 'csv_format' => TTi18n::gettext('CSV (Excel)')) );
