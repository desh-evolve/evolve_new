...
if ( !$permission->Check('report','enabled')
		OR !$permission->Check('report','mail_pay_slip') ) {
	$permission->Redirect( FALSE ); //Redirect
}

$smarty->assign('title', TTi18n::gettext($title = 'Mail Pay Slip'));  // See index.php
			...
			if ( ( ( isset($filter_data['transaction_start_date']) AND isset($filter_data['transaction_end_date']) ) OR isset($filter_data['pay_period_ids']) )
				...
				if ( $action == 'view_pay_stubs' ) {
					...
					if ( $pslf->getRecordCount() > 0 ) {
						...
						$output = $pslf->mail( $pslf, (bool)$filter_data['hide_employer_rows'] );
...
//                                exit();
	...
	default:
		...
		$smarty->display('report/MailPaySlip.tpl');
