...
$smarty->assign('title', TTi18n::gettext($title = 'Monthly Leave Taken ')); // See index.php
		...
		elseif ( $action == 'export' ) {
			...
			if ( isset($rows) AND isset($filter_columns) ) {
				...
				//Add the basic identifing columns.
                            ...
                            if( $filter_data['export_type'] == 'pdfMonthlyLeaveTaken'){
                                ...
                                $output = $tssr->MonthlyLeaveTakenReport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               
                                                                                            
                                if ( Debug::getVerbosity() < 11 ) {                                    
                                    Misc::FileDownloadHeader('MonthlyLeaveTakenReport.pdf', 'application/pdf', strlen($output));
                                    echo $output;
                                    exit;                           
                                } 
                            ...
                            if( $filter_data['export_type'] == 'pdfMonthlyLeaveBalance'){
//                                 echo '<pre>'; print_r($rows[0]['data']); echo '<pre>';  die;
                                 Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                
                                $tssr= TTnew( 'TimesheetDetailReport' );//new code                                
                                $output = $tssr->MonthlyLeavebalanceReport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code                               
                                 
                                
                                if ( Debug::getVerbosity() < 11 ) {                                    
                                    Misc::FileDownloadHeader('MonthlyLeaveBalance.pdf', 'application/pdf', strlen($output));
                                    echo $output;
                                    exit;                           
                                }  
...
*/
		...
		$filter_data['sort_direction_options'] = Misc::getSortDirectionArray();
                ...
                $filter_data['export_type_options'] = Misc::prependArray( array(  'pdfMonthlyLeaveTaken' => TTi18n::gettext('Monthly Leave Taken Report'), 'pdfMonthlyLeaveBalance' => TTi18n::gettext('Monthly Leave Balance Report')) );
