...
$smarty->assign('title', TTi18n::gettext($title = 'Overtime Summary Report')); // See index.php
...
switch ($action) {
	...
	case 'display_detailed_timesheet':
		...
		//from the MyTimeSheet page.
                $filter_data['date_type'] = "pay_period_ids";
		if ( isset($filter_data['print_timesheet']) AND $filter_data['print_timesheet'] >= 1 ) {
...
//print_r($tmp_sub_rows);
		...
		if ( $action == 'display_timesheet' ) {
			...
			if ( isset($rows) AND isset($filter_columns) ) {
                            if( $filter_data['export_type'] == 'otsummarysheet'){
                                 Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                
                                $tssr= TTnew( 'TimesheetDetailReport' );//new code    
                                //thilini
                                $output = $tssr->OverTimeSummarySheet($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code 
                            ...
                            elseif ($filter_data['export_type'] == 'otsummaryexcel') { //new code  added by Thusitha
                                
                                $tssr= TTnew( 'TimesheetDetailReport' );   
                                
                                $output = $tssr->OverTimeSummarySheetExcelExport($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code 
...
*/
		...
		$filter_data['sort_direction_options'] = Misc::getSortDirectionArray();
                ...
                $filter_data['export_type_options'] = Misc::prependArray( array( 'otsummarysheet' => TTi18n::gettext('OT Summary PDF Sheet'),'otsummaryexcel' => TTi18n::gettext('OT Summary Excel Sheet')) );
