...
$smarty->assign('title', TTi18n::gettext($title = 'Overtime Summary Report - Month Wise')); // See index.php
...
$columns = array(
    ...
    '-1170-hourly_wage' => TTi18n::gettext('Hourly Wage'),
											);
...
//		$ulf->getSearchByCompanyIdAndArrayCriteriaForOT( $current_company->getId(), $filter_data );
		...
		if ( $ulf->getRecordCount() > 0 ) {
			...
			if ( isset($filter_data['user_id']) ) {
				$udtlf->getDayReportByCompanyIdAndArrayCriteriaForOTAndOP( $current_company->getId(), $filter_data );
			}
			...
			foreach ($udtlf as $udt_obj ) {
				...
				$policy_id = 0;
//                                echo '<pre>';
//                                print_r("status_id".$status_id);
//                                print_r("type_id".$type_id);
				if ( $status_id == 10 AND $type_id == 10 ) {
					...
					$policy_id = $udt_obj->getColumn('over_time_policy_id');
                                        $overtime_policy_id = $udt_obj->getColumn('over_time_policy_id');
//                                        print_r("overtime_policy_id :".$overtime_policy_id." ");
				...
				if ( $column == 'worked_time' ) {
					...
					if ( isset($user_wage[$user_id]) ) {
						$tmp_rows[$pay_period_id][$user_id][$date_stamp]['actual_time_diff_wage'] = Misc::MoneyFormat( bcmul( TTDate::getHours($actual_time_diff), $user_wage[$user_id]), FALSE );
                                                $tmp_rows[$pay_period_id][$user_id][$date_stamp]['hourly_wage'] = $user_wage[$user_id];
					} else {
...
//                                        die;
					...
					if ( $udt_obj->getColumn('total_time') > 0 ) {
						...
						if ($status_id == 10 AND $type_id == 30) {
                                                  //  print_r($overtime_policy_id); //here change thilini
                                                    $tmp_rows[$pay_period_id][$user_id][$date_stamp]['over_time_policy_id'] = $overtime_policy_id;
							if ( isset($tmp_rows[$pay_period_id][$user_id][$date_stamp]['over_time']) ) {
								...
								$tmp_rows[$pay_period_id][$user_id][$date_stamp]['over_time'] = $udt_obj->getColumn('total_time');
                                                ...
                                                if ($status_id == 10 AND $type_id == 40) {
							if ( isset($tmp_rows[$pay_period_id][$user_id][$date_stamp]['op_time']) ) {
								$tmp_rows[$pay_period_id][$user_id][$date_stamp]['op_time'] += $udt_obj->getColumn('total_time');
							} else {
								$tmp_rows[$pay_period_id][$user_id][$date_stamp]['op_time'] = $udt_obj->getColumn('total_time');
							}
						}
...
//print_r($tmp_sub_rows);
		...
		if ( $action == 'export' ) {
			if ( isset($rows) AND isset($filter_columns) ) {
                            if( $filter_data['export_type'] == 'otsummarymonthsheet'){
                                 Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                
                                $tssr= TTnew( 'TimesheetDetailReport' );  
                                $output = $tssr->OverTimeSummaryMonthSheetEmployee($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code 
                            ...
                            } else if( $filter_data['export_type'] == 'otsummarydept'){
                                Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                
                                $tssr= TTnew( 'TimesheetDetailReport' );  
                                $output = $tssr->OverTimeSummaryMonthSheetDept($rows, $filter_columns, $filter_data, $current_user, $current_company);//new code 
...
*/
		...
		$filter_data['sort_direction_options'] = Misc::getSortDirectionArray();
                ...
                //FL ADDED FOR EXPORT TYPE  Misc::prependArray( array( 'csv' => TTi18n::gettext('CSV (Excel)'), 'pdfOTDetails' => TTi18n::gettext('OT Daily Monthly Report'), 'pdfDailyLate' => TTi18n::gettext('Daily Attendance / Late'), 'pdfMonthlyDetailAttendance' => TTi18n::gettext('Monthly Attendance Report'), 'pdfMonthlyDetailLate' => TTi18n::gettext('Monthly Late Report')) );
                $filter_data['export_type_options'] = Misc::prependArray( array( 'otsummarymonthsheet' => TTi18n::gettext('OT Summary - Employees'),
                                                                                 'otsummarydept' => TTi18n::gettext('OT Summary - Departments')));
