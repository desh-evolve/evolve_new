...
$smarty->assign('title', TTi18n::gettext($title = 'Over Time Report Monthly'));  // See index.php
...
switch ($action) {
	case 'export':
	case 'display_report':
		//Debug::setVerbosity(11); echo '<pre>';
...
*/
		...
		if ( $action == 'export'  ) {
				...
				Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                ...
                                $tssr= TTnew( 'TimesheetSummaryReport' );//new code                                
                     ...
                     if( $filter_data['export_type'] == 'pdfOTreport'){
						...
						Debug::Text('Exporting as PDF', __FILE__, __LINE__, __METHOD__,10);
                                ...
                                $filter_columns  = array(
                                						'employee_number' => '#',
                                                       	'full_name' => 'Full Name',                             
                                                        'over_time_policy-8' => 'Holiday OT', 
                                                        'over_time_policy-9' => 'Weekday OT', 
                                                        'over_time_policy-7' => 'Sunday OT',
                                                        'over_time_policy-6' => 'Saturday OT'
                                                        );
                                                        //'pay_period' => 'Pay Period',
//                               echo '<pre>'; print_r($filter_data); echo '<pre>'; die;
                                $tssr= TTnew( 'TimesheetSummaryReport' );//new code                                
                                $output = $tssr->Array2PDF($rows, $filter_columns, $current_user, $current_company,'Monthly OT Report',$filter_data);//new code                               
                                                                                               
                                if ( Debug::getVerbosity() < 11 ) {                                    
                                    Misc::FileDownloadHeader('MonthlyOverTimeReport.pdf', 'application/pdf', strlen($output));
                                    echo $output;
                                    exit;                           
                                }                        
                        }else {
	...
	default:
		...
		/***/
                //FL HIDED FOR MAKE REPORT ONLY FOR MONTHLY OT REPORT
                $filter_data['export_type_options'] = Misc::prependArray( array( 'pdfOTreport' => TTi18n::gettext('Monthly OT Report')) );
//                $filter_data['export_type_options'] = Misc::prependArray( array( 'csv' => TTi18n::gettext('CSV (Excel)'), 'pdfp' => TTi18n::gettext('PDF (PORTRAIT)'), 'pdfOTreport' => TTi18n::gettext('Monthly OT Report')) );
