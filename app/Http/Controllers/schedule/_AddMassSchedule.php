...
class _AddMassSchedule extends Controller
{
    ...
    public function index()
    {
        ...
        extract(FormVariables::GetVariables(
            array('action', 'id', 'data', 'filter_user_id')
        ));

        $data = Misc::preSetArrayValues($data, array('start_date_stamp', 'end_date_stamp', 'start_time', 'end_time'), NULL);

        if (isset($data)) {
            // Parse start and end full timestamps
            $data['start_full_time_stamp'] = TTDate::parseDateTime($data['start_date_stamp'] . ' ' . $data['start_time']);
            $data['end_full_time_stamp'] = TTDate::parseDateTime($data['end_date_stamp'] . ' ' . $data['end_time']);

            // Convert timestamps to MySQL datetime strings (IMPORTANT FIX)
            $data['start_time'] = date('Y-m-d H:i:s', $data['start_full_time_stamp']);
            $data['end_time'] = date('Y-m-d H:i:s', $data['end_full_time_stamp']);
            ...
            default:
                ...
                if ($action != 'submit' AND !is_array($data)) {
                    ...
                    $data = array(
                        ...
                        //'user_date_id' => $user_date_id,
                        //'user_full_name' => $user_full_name,
                        // Convert default timestamps to datetime strings here also
                        'start_time' => date('Y-m-d H:i:s', strtotime('08:00 AM')),
                        'parsed_start_time' => strtotime('08:00 AM'),
                        'end_time' => date('Y-m-d H:i:s', strtotime('05:00 PM')),
                        'parsed_end_time' => strtotime('05:00 PM'),
                        'total_time' => 3600 * 9,
                        'branch_id' => $user_default_branch,
                        'department_id' => $user_default_department,
