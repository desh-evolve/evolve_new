        ...
        switch ($action) {
            case 'submit':
                ...
                if ( TTDate::getDayDifference( $data['start_date_stamp'], $data['end_date_stamp']) > 31 ) {
                    ...
                    $sf->Validator->isTrue('date_stamp', FALSE, 'Date range exceeds the maximum of 31 days' );
                }

                if ( !( isset($filter_user_id) AND is_array($filter_user_id) AND count($filter_user_id) > 0 )  ) {
                    $sf->Validator->isTrue('user_id', FALSE, 'Please select at least one employee' );
                }
            ...
            default:
                ...
                $prepend_array_option = array( 0 => '--', -1 => '-- Default --' );
