    ...
    public function index($id = null) {
		...
		switch ($action) {
			case 'delete':
				...
				Redirect::Page( URLBuilder::getURL( array('id' => $data['id']), '/policy/accrual_policies/add') );
					...
					if ( $redirect == 0 ) {
						...
						if ( isset($ap_id) AND isset($data['recalculate']) AND $data['recalculate'] == 1 ) {
							...
							AND $data['recalculate_start_date'] < $data['recalculate_end_date']) {
								Redirect::Page( URLBuilder::getURL( array('action' => 'recalculate_accrual_policy', 'data' => array('accrual_policy_id' => $ap_id, 'start_date' => $data['recalculate_start_date'], 'end_date' => $data['recalculate_end_date']), 'next_page' => urlencode( URLBuilder::getURL( NULL, '/policy/accrual_policies') ) ), '/progress_bar_control'), FALSE );
							}
						}

						Redirect::Page( URLBuilder::getURL( NULL, '/policy/accrual_policies') );
