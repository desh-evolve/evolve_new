...
class EditPremiumPolicy extends Controller
{
    ...
    public function index($id = null) {
		...
		$viewData['title'] = 'Edit Premium Policy';
		...
		switch ($action) {
			case 'submit':
				...
				if ( $data['type_id'] == 10 OR $data['type_id'] == 100 ) {
					$ppf->setStartDate( $data['start_date'], FALSE );
					$ppf->setEndDate( $data['end_date'], FALSE );

					$ppf->setStartTime( $data['start_time'], FALSE );
					$ppf->setEndTime( $data['end_time'], FALSE );
				...
				if ( $ppf->isValid() ) {
					...
					if ( $ppf->isValid() ) {
						...
						Redirect::Page( URLBuilder::getURL( NULL, '/policy/premium_policies') );
