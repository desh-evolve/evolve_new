...
class AddMassPunch extends Controller
{
    ...
    public function index() {
		...
		switch($action){
			case 'submit':
				//dd(['filter_user_id' => $filter_user_id, 'pc_data' => $pc_data]);
				...
				if ( isset($filter_user_id) AND is_array($filter_user_id) AND count($filter_user_id) > 0 ) {
					Redirect::Page( URLBuilder::getURL( array('action' => 'add_mass_punch', 'filter_user_id' => $filter_user_id, 'data' => $pc_data ), '/progress_bar') );
				} else {
