...
if ( $saved_search_id == '' AND !isset($filter_data['columns']) ) {
	...
	if ( $sort_column == '' ) {
		$sort_column = $filter_data['sort_column'] = 'employee_number';
...
switch ($action) {
	...
	default:
		...
		//var_dump($users);
///---@eranda add code sorting problem employee number
		foreach ($users as $key => $row) {
			$volume[$key]  = $row['employee_number'];
		}
		if($sort_order == 'asc' and $sort_column =='employee_number') {
			array_multisort($volume, SORT_ASC, $users);
		}elseif($sort_order == 'desc' and $sort_column =='employee_number'){
			array_multisort($volume, SORT_DESC, $users);
