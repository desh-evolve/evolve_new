...
switch ($action) {
	case 'add':
		...
		Redirect::Page( URLBuilder::getURL(array('user_id' => $user_id, 'saved_search_id' => $saved_search_id ), 'EditUserDateUpdateForm.php', FALSE) );
	...
	case 'delete' OR 'undelete':
		if ( strtolower($action) == 'delete' ) {
			...
			$delete = FALSE;
                ...
                $ujlf = TTnew( 'UserDateUpdateFormListFactory' );
		...
		Redirect::Page( URLBuilder::getURL(array('user_id' => $user_id), 'UserDateUpdateForm.php') );
	...
	default:
		...
		if ( is_object($user_obj) ) {
			...
			if ( $permission->Check('wage','view')
				...
				foreach ($ujlf as $job) {
//                                    echo '<pre>';print_r($job); echo '<pre>'; die;
...
//                                    echo '<pre>';print_r($job_history); echo '<pre>'; die;
...
$smarty->display('users/UserDateUpdateForm.tpl');
?>