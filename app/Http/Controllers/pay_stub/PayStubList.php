...
class PayStubList extends Controller
{
	...
	public function index()
	{
		...
		if (
			!$permission->Check('pay_stub', 'enabled')
		...
		*/
		...
		$columns = array(
			...
			'-1090-transaction_date' => __('Transaction Date'),
		);
		...
		switch ($action) {
			...
			case 'mark_paid':
				...
				// Redirect::Page( URLBuilder::getURL( array('saved_search_id' => $saved_search_id, 'filter_pay_period_id' => $filter_pay_period_id, 'filter_user_id' => $filter_user_id), 'PayStubList.php') );
				return redirect(URLBuilder::getURL(NULL, '/attendance/paystubs'));
				// Redirect::Page( URLBuilder::getURL( array('saved_search_id' => $saved_search_id, 'filter_pay_period_id' => $filter_pay_period_id, 'filter_user_id' => $filter_user_id), '../pay_stub/PayStubList.php') );
				break;
			case 'mark_unpaid':
				//Debug::setVerbosity(11);
			...
			case 'search_form_search':
				...
				$ugdf = new UserGenericDataFactory();
				$saved_search_id = $ugdf->searchFormDataHandler($action, $filter_data, URLBuilder::getURL(NULL, 'PayStubList.php'));
			default:
