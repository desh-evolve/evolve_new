		...
		*/
		...
		switch ($action) {
			...
			case 'apply_preset':
				...
				if ( in_array( $current_user->getId(), (array)$src_user_id) ) {
					...
					if ( $current_user_failed == TRUE ) {
						$pcf->Validator->isTrue( 'user',
												FALSE,
												'You can not unassign yourself from a permission group, assign yourself to a new group instead') ;
					}
				}
				...
				if ( $pcf->isValid() ) {
					...
					if ( $pcf->isValid() ) {
						...
						Redirect::Page( URLBuilder::getURL( array(), '/permission_control') );
			...
			default:
				...
				$section_groups = Misc::prependArray( array( -1 => '-- None --'), $pf->getOptions('section_group') );
				...
				$preset_options = Misc::prependArray( array( -1 => '--'), $pf->getOptions('preset') );
